...
분석 과정, 선택 이유, 수정 내역, A/B 표기, 제목, 안내 문구, 코드블록을 출력하지 않는다. 통합된 RP 본문만 출력한다.`;
    ...
    function showMixerModal(bundle) {
            ...
            listEl.innerHTML = `
                <div class="${SCRIPT_NS}-answer-info">
                    <div class="${SCRIPT_NS}-answer-info-main">
                        ...
                        ${reroll ? '<span class="badge reroll">reroll</span>' : '<span class="badge original">original</span>'}
        ...
        function renderAll() {
            ...
            meta.textContent = `총 ${total}개 리롤 · 현재 답변 ${current + 1}/${total}${promptPreview ? ` · 입력: ${promptPreview}` : ''}`;
