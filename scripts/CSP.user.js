...
(function () {
    ...
    const SECRET_SETTINGS_KEY = `${CSP_PREFIX}_secret_settings_v1`;
    const SECRET_SETTING_FIELDS = Object.freeze([
        'googleApiKey',
        'deepseekApiKey',
        'firebaseConfigJson',
        'naiApiKey'
...
[Gold Pattern A — 같은 장소 유형의 세계 차이]
세계관 설정이 현대 대도시이고 target이 호텔 객실이면 해당 시대·문화권의 generic visual language만 사용한다. 세계관 설정이 근대풍 항구 도시이고 시각 설정이 오래된 저택이면 그 장소와 시대에 맞는 공간 특성만 사용한다. 단, 유명 랜드마크처럼 직접 근거 없는 구체물은 추가하지 않는다.
...
- none이면 fragmentTags, tags, caption은 모두 빈 문자열이다.`;
    ...
    function getSecretSettings() {
        if (secretSettingsCache && typeof secretSettingsCache === 'object') return { ...secretSettingsCache };
        try {
            if (typeof GM_getValue !== 'function') return {};
            const raw = GM_getValue(SECRET_SETTINGS_KEY, '{}');
            const parsed = typeof raw === 'string' ? safeJsonParse(raw, {}) : raw;
            secretSettingsCache = parsed && typeof parsed === 'object' ? parsed : {};
            return { ...secretSettingsCache };
        } catch (err) {
            console.warn('[Crack Scene Painter] secure settings read failed:', err);
            return {};
        }
    }

    function extractSecretSettings(settings = {}) {
        return SECRET_SETTING_FIELDS.reduce((result, field) => {
    ...
    function stripSecretSettings(settings = {}) {
        ...
        SECRET_SETTING_FIELDS.forEach(field => delete publicSettings[field]);
        return publicSettings;
    }

    function saveSecretSettings(settings = {}) {
        if (typeof GM_setValue !== 'function') {
            throw new Error('사용자스크립트 보안 저장소를 사용할 수 없어요. 스크립트 권한을 다시 확인해줘.');
        }
        const secrets = extractSecretSettings(settings);
        if (Object.keys(secrets).length) {
            GM_setValue(SECRET_SETTINGS_KEY, JSON.stringify(secrets));
        } else if (typeof GM_deleteValue === 'function') {
            GM_deleteValue(SECRET_SETTINGS_KEY);
        } else {
            GM_setValue(SECRET_SETTINGS_KEY, '{}');
        }
        secretSettingsCache = { ...secrets };
    }

    function migrateLegacySecretsToUserscriptStorage() {
        const saved = getLocalJsonStorage(GLOBAL_SETTINGS_KEY, {});
        const legacySecrets = extractSecretSettings(saved);
        if (!Object.keys(legacySecrets).length) return;

        const mergedSecrets = { ...getSecretSettings(), ...legacySecrets };
        saveSecretSettings(mergedSecrets);
        setLocalJsonStorage(GLOBAL_SETTINGS_KEY, stripSecretSettings(saved));
        debugLog('legacy API credentials migrated to userscript storage');
    }

    function getGlobalSettings() {
        const saved = getLocalJsonStorage(GLOBAL_SETTINGS_KEY, {});
        const legacyRoom = getLocalJsonStorage(getRoomSettingsKey(), {});
        return normalizeGlobalSettings({ ...saved, ...getSecretSettings() }, { legacyRoom });
    }

    function saveGlobalSettings(settings) {
        try {
            const normalized = normalizeGlobalSettings(settings);
            // 보안 저장이 실패하면 localStorage를 먼저 지우지 않는다.
            saveSecretSettings(normalized);
            setLocalJsonStorage(GLOBAL_SETTINGS_KEY, stripSecretSettings(normalized));
        } catch (err) {
    ...
    function migrateRemovedVertexProviderSettings() {
        try {
            ...
            const rawSecrets = getSecretSettings();
            ...
            const hadVertexFields = ['vertexProjectId', 'vertexLocation', 'vertexAccessToken']
                .some(key => rawSaved?.[key] !== undefined || rawSecrets?.[key] !== undefined);
            if (!hadVertexProvider && !hadVertexFields) return;

            const hasFirebaseConfig = !!String(rawSecrets?.firebaseConfigJson || rawSaved?.firebaseConfigJson || '').trim();
            ...
            const cleanedSecrets = { ...rawSecrets };
            delete cleanedSecrets.vertexAccessToken;
            saveSecretSettings(cleanedSecrets);
            setLocalJsonStorage(GLOBAL_SETTINGS_KEY, stripSecretSettings(rawSaved));
...
{"scenePrompt":"tag1, tag2, tag3"}`
    ...
    // =========================================================================
    // Desktop App · Auto Theme / Korean UI / Field Grouping
    // Old modal/gallery/settings DOM is not reused. Core functions/data only.
    // =========================================================================
    const CSP_V35_UI_KEY = `${CSP_PREFIX}_v35_ui_state`;
    let cspV35Session = null;
    let cspV35Root = null;
    let cspV35CanvasObserver = null;
    ...
    function getV35UiState() {
        const base = {studio:{tool:'',inspector:'prompt',advanced:false},settings:{page:'connection',worldTab:'world'},gallery:{query:'',sort:'newest'}};
        const rawSaved = getLocalJsonStorage(CSP_V35_UI_KEY, {});
        // 수동 테마 상태는 폐기. Crack 사이트 테마를 항상 자동으로 따른다.
...
#csp-v35-root *,#csp-v35-root *:before,#csp-v35-root *:after{box-sizing:border-box}#csp-v35-root button,#csp-v35-root input,#csp-v35-root select,#csp-v35-root textarea{font:inherit;color:inherit}#csp-v35-root button{cursor:pointer}#csp-v35-root button:disabled{opacity:.45;cursor:default}
#csp-v35-root .v35-app{width:clamp(1120px,calc(100vw - 64px),1600px);height:clamp(680px,calc(100vh - 64px),960px);background:var(--bg);border:1px solid var(--line);border-radius:var(--r4);box-shadow:var(--shadow);overflow:hidden;display:flex;flex-direction:column}
#csp-v35-root .v35-app.settings{width:clamp(1000px,calc(100vw - 96px),1280px);height:clamp(640px,calc(100vh - 80px),900px)}#csp-v35-root .v35-app.gallery{width:min(calc(100vw - 64px),1760px);height:calc(100vh - 64px)}
#csp-v35-root .v35-top{height:52px;flex:0 0 52px;background:var(--sf);border-bottom:1px solid var(--line);display:flex;align-items:center;gap:var(--s2);padding:0 var(--s4)}#csp-v35-root .v35-brand{font-weight:600;font-size:14px;display:flex;gap:var(--s2);align-items:center}#csp-v35-root .v35-brand small{font-size:11px;color:var(--tx3);font-weight:400}#csp-v35-root .grow{flex:1}
#csp-v35-root .btn{height:32px;padding:0 var(--s4);border:1px solid var(--line);border-radius:var(--r1);background:var(--sf2);color:var(--tx);display:inline-flex;align-items:center;justify-content:center;gap:var(--s2);white-space:nowrap;font-weight:500}#csp-v35-root .btn:hover{background:var(--sf3)}#csp-v35-root .btn.ghost{background:transparent;border-color:transparent;color:var(--tx2)}#csp-v35-root .btn.ghost:hover{background:var(--hover);color:var(--tx)}#csp-v35-root .btn.primary{height:40px;background:var(--acs);border-color:transparent;color:#fff;font-weight:600}#csp-v35-root .btn.danger{background:transparent;color:var(--danger)}#csp-v35-root .btn.tiny{height:24px;padding:0 var(--s3);font-size:11px}
#csp-v35-root .iconbtn{width:32px;height:32px;border:1px solid transparent;border-radius:var(--r1);background:transparent;color:var(--tx2);display:grid;place-items:center}#csp-v35-root .iconbtn:hover{background:var(--hover);color:var(--tx)}#csp-v35-root .iconbtn.active{background:var(--soft);color:var(--ac)}
#csp-v35-root .input,#csp-v35-root .select,#csp-v35-root .ta{width:100%;border:1px solid var(--line);border-radius:var(--r2);background:var(--sf2);outline:none;color:var(--tx)}#csp-v35-root .input,#csp-v35-root .select{height:36px;padding:0 var(--s4)}#csp-v35-root .ta{padding:10px 12px;line-height:20px;resize:vertical}#csp-v35-root .ta.compact{min-height:80px;max-height:160px}#csp-v35-root .ta.default{min-height:128px;max-height:320px}#csp-v35-root .ta.editor{min-height:224px;max-height:560px;font:11px/18px var(--mono)}
#csp-v35-root .field{display:flex;flex-direction:column;gap:var(--s2);min-width:0}#csp-v35-root .field label{font-size:12px;line-height:16px;color:var(--tx2)}#csp-v35-root .help{font-size:11px;line-height:16px;color:var(--tx3);max-width:68ch}#csp-v35-root .mono{font:11px/18px var(--mono)}#csp-v35-root .grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--s4)}#csp-v35-root .grid3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:var(--s4)}
#csp-v35-root .section{padding:var(--s5) 0;border-bottom:1px solid var(--line2);display:flex;flex-direction:column;gap:var(--s4)}#csp-v35-root .section:last-child{border-bottom:0}#csp-v35-root .section-title{font-size:15px;line-height:22px;font-weight:600;display:flex;align-items:center;gap:var(--s2)}#csp-v35-root .badge{display:inline-flex;align-items:center;height:20px;padding:0 var(--s2);font-size:10px;font-weight:700;border:1px solid var(--line);border-radius:6px;color:var(--tx3)}#csp-v35-root .badge.accent{background:var(--soft);border-color:transparent;color:var(--ac)}#csp-v35-root .badge.ok{color:var(--ok)}
#csp-v35-root .studio{flex:1;min-height:0;display:grid;grid-template-columns:48px minmax(0,1fr) 360px}#csp-v35-root .studio.panel{grid-template-columns:48px 320px minmax(0,1fr) 360px}#csp-v35-root .rail{background:var(--sf);border-right:1px solid var(--line);display:flex;flex-direction:column;align-items:center;gap:var(--s2);padding:var(--s3) 0}#csp-v35-root .studio.panel .rail{border-right:0}#csp-v35-root .rail .iconbtn{width:36px;height:36px;font-size:16px}
#csp-v35-root .context-panel{background:var(--sf);border-right:1px solid var(--line);padding:var(--s5);min-height:0;overflow:auto;overscroll-behavior:contain}#csp-v35-root .panel-head{display:flex;align-items:center;gap:var(--s2);margin-bottom:var(--s5)}#csp-v35-root .panel-head h3{margin:0;font-size:14px;font-weight:600}
#csp-v35-root .output{background:var(--sf2);min-width:0;min-height:0;overflow:hidden;display:flex;flex-direction:column}#csp-v35-root .canvas-wrap{flex:1;min-height:0;display:grid;place-items:center;padding:var(--s5);position:relative;overflow:hidden}#csp-v35-root .canvas{width:auto;height:auto;max-width:none;max-height:none;aspect-ratio:auto;background:transparent;border:0;border-radius:0;overflow:hidden;display:grid;place-items:center;position:relative}#csp-v35-root .canvas img{width:100%;height:100%;max-width:none;max-height:none;object-fit:contain;display:block;border:1px solid var(--line);border-radius:var(--r3)}#csp-v35-root .canvas:has(.canvas-empty){aspect-ratio:var(--iw,832)/var(--ih,1216);width:min(100%,420px);height:auto;background:#08090d;border:1px solid var(--line);border-radius:var(--r3)}#csp-v35-root .canvas-empty{text-align:center;color:var(--tx3);padding:var(--s7)}#csp-v35-root .canvas-empty strong{display:block;color:var(--tx);font-size:14px;margin-bottom:var(--s2)}#csp-v35-root .canvas-tools{position:absolute;right:var(--s3);top:var(--s3);display:flex;gap:var(--s1);opacity:0}#csp-v35-root .canvas:hover .canvas-tools{opacity:1}#csp-v35-root .glass{background:rgba(12,14,20,.62)!important;border:1px solid rgba(255,255,255,.14)!important;color:#e9ebf1!important;backdrop-filter:blur(6px)}
#csp-v35-root .loading{position:absolute;inset:0;background:rgba(8,9,13,.72);backdrop-filter:blur(3px);display:grid;place-items:center;z-index:5}#csp-v35-root .loading-card{width:min(320px,80%);padding:20px;text-align:center;background:rgba(15,17,24,.9);border:1px solid rgba(255,255,255,.1);border-radius:var(--r3);color:#e9ebf1}#csp-v35-root .spinner{width:18px;height:18px;border:2px solid rgba(255,255,255,.2);border-top-color:#fff;border-radius:50%;margin:0 auto 8px;animation:v35spin .75s linear infinite}@keyframes v35spin{to{transform:rotate(360deg)}}
#csp-v35-root .film{height:88px;flex:0 0 88px;background:var(--sf);border-top:1px solid var(--line);display:flex;align-items:center;gap:var(--s2);padding:var(--s3) var(--s4)}#csp-v35-root .film-label{width:74px;font-size:11px;color:var(--tx3);text-transform:uppercase}#csp-v35-root .thumb{width:46px;height:66px;border:1px solid var(--line);border-radius:var(--r1);background:var(--sf3);overflow:hidden;padding:0}#csp-v35-root .thumb.active{border-color:var(--ac);box-shadow:0 0 0 2px var(--soft)}#csp-v35-root .thumb img{width:100%;height:100%;object-fit:cover}
#csp-v35-root .inspector{background:var(--sf);border-left:1px solid var(--line);display:flex;flex-direction:column;min-width:0;min-height:0;overflow:hidden}#csp-v35-root .inspector-tabs{height:44px;display:flex;padding:0 var(--s3);border-bottom:1px solid var(--line)}#csp-v35-root .tab{height:44px;padding:0 var(--s4);border:0;border-bottom:2px solid transparent;background:transparent;color:var(--tx3)}#csp-v35-root .tab.active{border-bottom-color:var(--ac);color:var(--tx);font-weight:600}#csp-v35-root .inspector-body{flex:1;min-height:0;overflow:auto;padding:var(--s5);overscroll-behavior:contain}
#csp-v35-root .prompt-block{padding:var(--s4) 0;border-bottom:1px solid var(--line2)}#csp-v35-root .prompt-block:first-child{padding-top:0}#csp-v35-root .prompt-head{display:flex;align-items:center;gap:var(--s2);margin-bottom:var(--s2);font-size:12px;color:var(--tx2)}#csp-v35-root .prompt-preview{font:11px/18px var(--mono);color:var(--tx2);display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;overflow:hidden}
#csp-v35-root .chipview{display:flex;flex-wrap:wrap;gap:3px}
#csp-v35-root .tg{padding:2px 6px;border-radius:6px;background:var(--sf3);color:var(--tx2);font:10px/14px var(--mono)}
#csp-v35-root button.tg.more{border:0;background:transparent;color:var(--tx3);cursor:pointer}
#csp-v35-root button.tg.more:hover{color:var(--ac)}
#csp-v35-root .slotline{min-height:30px;display:flex;align-items:center;gap:var(--s2);padding:0 var(--s3);border:1px solid var(--line);border-radius:var(--r2);background:var(--sf2);font-size:11px;color:var(--tx3);cursor:pointer;list-style:none}
#csp-v35-root .slotline::-webkit-details-marker{display:none}
#csp-v35-root .slotline:hover{color:var(--tx2)}
#csp-v35-root .slotline.is-static{cursor:default}
#csp-v35-root .slotfold[open] .slotline{display:none}
#csp-v35-root .slotfold textarea{width:100%}

#csp-v35-root .rowitem{width:100%;min-height:44px;border:0;border-bottom:1px solid var(--line2);background:transparent;color:var(--tx2);display:flex;align-items:center;gap:var(--s3);text-align:left;padding:var(--s3) 0}#csp-v35-root .rowitem:hover{color:var(--tx)}#csp-v35-root .rowitem .main{flex:1;min-width:0}#csp-v35-root .rowitem .name{font-weight:600;display:block;color:var(--tx)}#csp-v35-root .rowitem .meta{font-size:11px;color:var(--tx3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}
#csp-v35-root .bottom{height:60px;flex:0 0 60px;background:var(--sf);border-top:1px solid var(--line);display:flex;align-items:center;gap:var(--s2);padding:0 var(--s4)}
#csp-v35-root .settings-body{flex:1;min-height:0;display:grid;grid-template-columns:200px minmax(0,1fr)}#csp-v35-root .settings-nav{background:var(--sf);border-right:1px solid var(--line);padding:var(--s4);display:flex;flex-direction:column;gap:var(--s1)}#csp-v35-root .navbtn{height:40px;border:0;border-radius:var(--r2);background:transparent;color:var(--tx2);text-align:left;padding:0 var(--s4)}#csp-v35-root .navbtn:hover{background:var(--hover);color:var(--tx)}#csp-v35-root .navbtn.active{background:var(--soft);color:var(--tx);font-weight:600}#csp-v35-root .settings-content{overflow:auto;padding:var(--s6) var(--s7);overscroll-behavior:contain}#csp-v35-root .settings-page{max-width:920px}#csp-v35-root .page-title{font-size:20px;line-height:26px;font-weight:600;margin:0}#csp-v35-root .page-desc{font-size:12px;color:var(--tx3);margin:4px 0 16px}
#csp-v35-root .master{display:grid;grid-template-columns:290px minmax(0,1fr);gap:var(--s5);min-height:520px}#csp-v35-root .list{border-right:1px solid var(--line);padding-right:var(--s5)}#csp-v35-root .listbtn{width:100%;min-height:48px;border:0;border-radius:var(--r2);background:transparent;color:var(--tx2);text-align:left;padding:var(--s3) var(--s4);display:flex;align-items:center;gap:var(--s3)}#csp-v35-root .listbtn:hover{background:var(--hover);color:var(--tx)}#csp-v35-root .listbtn.active{background:var(--soft);color:var(--tx)}
#csp-v35-root .gallery-body{flex:1;min-height:0;overflow:auto;padding:var(--s6);overscroll-behavior:contain}#csp-v35-root .gallery-toolbar{display:flex;align-items:center;gap:var(--s2);margin-bottom:var(--s5)}#csp-v35-root .gallery-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:var(--s5)}#csp-v35-root .gimg{width:100%;aspect-ratio:832/1216;border:1px solid var(--line);border-radius:var(--r3);background:var(--sf3);overflow:hidden;padding:0;display:grid;place-items:center;color:var(--tx3)}#csp-v35-root .gimg img{width:100%;height:100%;object-fit:cover}#csp-v35-root .gtitle{font-size:12px;font-weight:600;margin-top:var(--s2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}#csp-v35-root .gmeta{font-size:11px;color:var(--tx3);display:flex;gap:var(--s2);align-items:center}
#csp-v35-root .viewer{position:absolute;inset:0;z-index:20;background:var(--scrim);display:flex;flex-direction:column}#csp-v35-root .viewer-main{flex:1;min-height:0;display:grid;place-items:center;overflow:hidden}#csp-v35-root .viewer-img{max-width:calc(100vw - 160px);max-height:calc(100vh - 180px);transform-origin:center;cursor:grab;user-select:none}#csp-v35-root .viewer-footer{height:64px;background:var(--sf);border-top:1px solid var(--line);display:flex;align-items:center;gap:var(--s4);padding:0 var(--s5)}
#csp-v35-root .toaststack{position:absolute;right:20px;bottom:20px;z-index:50;display:flex;flex-direction:column;gap:6px}#csp-v35-root .toast{width:300px;padding:10px;border:1px solid var(--line);border-radius:var(--r3);background:var(--sf);box-shadow:var(--shadow);font-size:12px}#csp-v35-root .toast.error{border-color:var(--danger)}#csp-v35-root .toast.success{border-color:var(--ok)}
/* compact desktop density: keep readability, remove dead space */
#csp-v35-root .v35-app{width:min(calc(100vw - 48px),1480px);height:min(calc(100vh - 48px),860px);min-width:1120px;min-height:620px}
#csp-v35-root .v35-app.settings{width:min(calc(100vw - 72px),1160px);height:min(calc(100vh - 64px),820px);min-width:980px;min-height:600px}
#csp-v35-root .v35-app.gallery{width:min(calc(100vw - 48px),1560px);height:calc(100vh - 48px)}
#csp-v35-root .v35-top{height:46px;flex-basis:46px;padding:0 10px;gap:6px}
#csp-v35-root .btn{height:30px;padding:0 10px;gap:6px}#csp-v35-root .btn.primary{height:36px}#csp-v35-root .iconbtn{width:30px;height:30px}
#csp-v35-root .input,#csp-v35-root .select{height:34px;padding:0 10px}#csp-v35-root .ta{padding:8px 10px;line-height:19px}#csp-v35-root .ta.compact{min-height:64px;max-height:140px}#csp-v35-root .ta.default{min-height:104px;max-height:260px}#csp-v35-root .ta.editor{min-height:180px;max-height:460px}
#csp-v35-root .section{padding:12px 0;gap:8px}#csp-v35-root .section-title{font-size:14px;line-height:20px}
#csp-v35-root .studio{grid-template-columns:44px minmax(0,1fr) 326px}#csp-v35-root .studio.panel{grid-template-columns:44px 284px minmax(0,1fr) 326px}
#csp-v35-root .rail{gap:4px;padding:6px 0}#csp-v35-root .rail .iconbtn{width:34px;height:34px}
#csp-v35-root .context-panel{padding:12px}#csp-v35-root .panel-head{margin-bottom:10px}
#csp-v35-root .canvas-wrap{padding:12px}#csp-v35-root .canvas-empty{padding:16px}
#csp-v35-root .film{height:72px;flex-basis:72px;padding:6px 10px}#csp-v35-root .film-label{width:66px}#csp-v35-root .thumb{width:38px;height:54px}
#csp-v35-root .inspector-tabs{height:38px;padding:0 6px}#csp-v35-root .tab{height:38px;padding:0 10px}#csp-v35-root .inspector-body{padding:12px}
#csp-v35-root .prompt-block{padding:10px 0}#csp-v35-root .rowitem{min-height:38px;padding:6px 0}
#csp-v35-root .bottom{height:52px;flex-basis:52px;padding:0 10px}
#csp-v35-root .settings-body{grid-template-columns:176px minmax(0,1fr)}#csp-v35-root .settings-nav{padding:10px 8px}#csp-v35-root .navbtn{height:36px;padding:0 10px}
#csp-v35-root .settings-content{padding:16px 20px}#csp-v35-root .settings-page{max-width:860px}#csp-v35-root .page-title{font-size:18px;line-height:24px}#csp-v35-root .page-desc{margin:2px 0 12px}
#csp-v35-root .master{grid-template-columns:250px minmax(0,1fr);gap:12px;min-height:440px}#csp-v35-root .list{padding-right:12px}#csp-v35-root .listbtn{min-height:42px;padding:6px 10px}
#csp-v35-root .gallery-body{padding:16px}#csp-v35-root .gallery-toolbar{margin-bottom:12px}#csp-v35-root .gallery-grid{grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:12px}
#csp-v35-root .viewer-footer{height:52px;padding:0 12px;gap:10px}
#csp-v35-root .setting-row{grid-template-columns:minmax(160px,240px) minmax(250px,1fr)!important;gap:16px!important;padding:10px 0!important}
#csp-v35-root .tool-actions{padding:8px 0!important;margin-top:8px!important}
/* 한 필드 안은 가깝게, 필드끼리는 확실히 분리한다. */
#csp-v35-root .settings-page .field{gap:4px}
#csp-v35-root .settings-page .field label{font-weight:600;color:var(--tx2)}
#csp-v35-root .settings-page .section{gap:14px}
#csp-v35-root .settings-page .grid2,#csp-v35-root .settings-page .grid3{row-gap:14px}
#csp-v35-root .settings-page .section-title{margin-bottom:1px}
#csp-v35-root .settings-page .setting-row{border-bottom:0}
@media(max-width:1179px){#csp-v35-root .v35-app{display:none}#csp-v35-root:after{content:"Scene Painter PC판은 더 넓은 화면이 필요합니다.";padding:24px;border:1px solid var(--line);border-radius:12px;background:var(--sf);color:var(--tx)}}`;
    ...
    function fitV35CanvasToImage(root=cspV35Root){
        const wrap=root?.querySelector?.('.canvas-wrap');
        const canvas=root?.querySelector?.('#v35-canvas');
        const img=root?.querySelector?.('#v35-main-img');
        ...
        const apply=()=>{
            ...
            canvas.style.setProperty('--iw',String(iw));
            canvas.style.setProperty('--ih',String(ih));
        };
    ...
    function bindV35CanvasFit(root=cspV35Root){
        try{cspV35CanvasObserver?.disconnect()}catch(_){}
        cspV35CanvasObserver=null;

        const wrap=root?.querySelector?.('.canvas-wrap');
        if(!wrap)return;

        fitV35CanvasToImage(root);

        if(typeof ResizeObserver==='function'){
            cspV35CanvasObserver=new ResizeObserver(()=>fitV35CanvasToImage(root));
            cspV35CanvasObserver.observe(wrap);
        }
    }

    function closeV35(){stopV35ThemeSync();try{cspV35CanvasObserver?.disconnect()}catch(_){}cspV35CanvasObserver=null;cspV35RefineDialog?.close?.(null);cspV35RefineDialog=null;cspV35Root?.remove();cspV35Root=null;cspV35Viewer=null;document.documentElement.style.overflow=''}
    function exitV35Studio(){closeV35();cspV35Session=null}
    function mountV35(kind=''){ensureV35Styles();closeV35();const root=document.createElement('div');root.id='csp-v35-root';root.className=v35ThemeClass();const app=document.createElement('div');app.className=`v35-app ${kind}`.trim();root.appendChild(app);document.body.appendChild(root);document.documentElement.style.overflow='hidden';cspV35Root=root;applyV35Theme(root);startV35ThemeSync();root.addEventListener('mousedown',e=>{if(e.target===root)closeV35()});return{root,app}}

    // ── 할당량 · 예산 표시용 헬퍼 (표시 전용, 생성 로직과 무관) ──
    function v35EstimateTokens(text){
    ...
    function v35PromptBudget(s=cspV35Session){
        const cap=getNaiModelCapability(v35Model())||{};
        const ps=s?.promptState||{};
        const scene=v35EstimateTokens(ps.finalPrompt);
        const chars=(ps.charPrompts||[]).map(c=>({name:c.name||'인물',tokens:v35EstimateTokens(c.prompt)}));
        ...
        return {shared,scene,chars,used,limit,percent,
            status:percent>100?'over':(percent>=90?'tight':'ok'),
            charCount:chars.length,maxCharacters:Number(cap.maxCharacters)||6};
    }

    function v35QuotaView(s=cspV35Session){
        const q=s?.quota;
        ...
        return {mode:'battery',percent,status:q.status,
    ...
    function v35RefCount(s=cspV35Session){
    ...
    function v35EffectiveAreaForAnlas(settings={}){
        const width=Math.max(1,Number(settings.width||832));
        const height=Math.max(1,Number(settings.height||1216));
        const area=width*height;
        const longSide=Math.max(width,height);
        const shortSide=Math.min(width,height);
        if(longSide<=1024&&shortSide<=1024&&area<=1024*1024&&area>=512*768){
            return 832*1216;
        }
        return area;
    }

    function v35EstimateV45BaseAnlasPerImage(settings={}){
        const NORMAL_AREA=832*1216;
        const steps=Math.max(1,Number(settings.steps||28));
        const effectiveArea=v35EffectiveAreaForAnlas(settings);
        const baseAt23=Math.max(1,Math.round(17*(effectiveArea/NORMAL_AREA)));
        const stepFactor=Math.max(0.25,1+((steps-23)*0.032));
        return Math.max(1,Math.round(baseAt23*stepFactor));
    }

    function v35IsLegacyOpusFree(settings={},tier=''){
        const width=Math.max(1,Number(settings.width||832));
        const height=Math.max(1,Number(settings.height||1216));
        const steps=Math.max(1,Number(settings.steps||28));
        const samples=Math.max(1,Number(settings.nSamples||settings.n_samples||1));
        return /opus/i.test(String(tier||''))&&width*height<=1024*1024&&steps<=28&&samples===1;
    }

    function v35EstimatedAnlas(s=cspV35Session){
        const settings=s?.generation||{};
        const model=v35Model();
        const cap=getNaiModelCapability(model)||{};
        const family=cap.family||'v5';
        const tier=String(s?.quota?.tier||'unknown');
        const samples=Math.max(1,Number(settings.nSamples||settings.n_samples||1));
        const refCount=cap.supportsPreciseReference?v35RefCount(s):0;
        const preciseExtra=refCount*PRECISE_REFERENCE_EXTRA_ANLAS;
        const basePerImageV45=v35EstimateV45BaseAnlasPerImage(settings);
        const basePerImage=family==='v5'?Math.max(1,Math.round(basePerImageV45*1.5)):basePerImageV45;
        const baseGenerationFree=family==='v4.5'&&v35IsLegacyOpusFree(settings,tier);
        const baseTotal=baseGenerationFree?0:(basePerImage*samples);
        const total=Math.max(0,baseTotal+preciseExtra);
        const quota=v35QuotaView(s);
        const eligibleV5Quota=family==='v5'&&isV5SharedTrialEligible(settings,model);
        const nowUsesBattery=eligibleV5Quota&&quota.mode==='battery'&&(quota.status==='available'||quota.status==='low');
        const batteryUnknown=eligibleV5Quota&&(quota.mode==='loading'||quota.mode==='unknown');
        const needsAnlasNow=total>0&&!(nowUsesBattery||baseGenerationFree);
        const costLine=total<=0
            ? '0 Anlas'
            : (nowUsesBattery
                ? `배터리 사용 · Anlas 사용 시 예상 ${total} Anlas`
                : (batteryUnknown
                    ? `Anlas 기준 예상 ${total} Anlas`
                    : `예상 ${total} Anlas`));
        const chipText=total<=0?'0':`예상 ${total}`;
        const hint=[
            `${Math.max(1,Number(settings.width||832))}×${Math.max(1,Number(settings.height||1216))}`,
            `${Math.max(1,Number(settings.steps||28))} steps`,
            samples>1?`${samples}장 생성`:'1장 생성',
            family==='v5'?'V5 1.5배 기준':'V4.5 기준'
        ];
        if(baseGenerationFree)hint.push('Opus 무료 조건으로 기본 생성비 0');
        if(preciseExtra>0)hint.push(`Precise Reference ${refCount}장 +${preciseExtra} Anlas`);
        if(batteryUnknown)hint.push('배터리 적용 여부 확인 전');
        return {family,tier,samples,refCount,preciseExtra,basePerImage,baseTotal,total,baseGenerationFree,eligibleV5Quota,nowUsesBattery,batteryUnknown,needsAnlasNow,costLine,chipText,hint:hint.join(' · ')};
    }

    function v35GaugeHtml({label='',right='',percent=0,tone='ok',foot='',thin=false,segments=null}={}){
        const p=Math.max(0,Math.min(100,Number(percent)||0));
        const fill=Array.isArray(segments)&&segments.length
            ? segments.map(seg=>`<i class="${escapeHtml(seg.tone||'')}" style="width:${Math.max(0,Math.min(100,seg.percent))}%"></i>`).join('')
            : `<i class="${escapeHtml(tone)}" style="width:${p}%"></i>`;
        return `<div class="gauge">
${label||right?`<div class="gtop">${label?`<span class="glab">${label}</span>`:''}${right?`<span class="gr">${right}</span>`:''}</div>`:''}
<div class="bar${thin?' thin':''}">${fill}</div>
${foot?`<div class="gfoot ${tone==='ok'?'':tone}">${foot}</div>`:''}
</div>`;
    }

    async function refreshV35Quota(){
        if(!cspV35Session)return;
        try{
            const info=await fetchNaiAnlasBalance();
            if(!cspV35Session)return;
            cspV35Session.quota={...(info?.quota||{status:'unavailable'}),tier:info?.tier};
        }catch(_){
            if(cspV35Session)cspV35Session.quota={status:'unavailable'};
        }
        if(cspV35Root?.dataset.view==='studio')renderV35Studio();
    }

    function v35QuotaChipHtml(){
        const v=v35QuotaView();
        if(v.mode==='loading')return `<span class="q-chip" id="v35-quota-chip"><span class="minibar"><i style="width:0%"></i></span>확인 중</span>`;
        if(v.mode==='anlas')return `<span class="q-chip" id="v35-quota-chip" title="Opus 등급이 아니라 V5 배터리는 적용되지 않아">💠 Anlas</span>`;
        if(v.mode==='unknown')return `<span class="q-chip" id="v35-quota-chip" title="구독 정보를 읽지 못했어. 생성은 그대로 돼.">🔋 확인 불가</span>`;
        const tone=v.status==='exhausted'?'dgr':(v.status==='low'?'warn':'');
        return `<span class="q-chip ${tone}" id="v35-quota-chip"><span class="minibar"><i class="${tone}" style="width:${v.percent}%"></i></span><b>${v.percent}%</b></span>`;
    }

    function v35BudgetChipHtml(){
        const b=v35PromptBudget();
        const tone=b.status==='over'?'dgr':(b.status==='tight'?'warn':'');
        return `<span class="q-chip ${tone}" id="v35-budget-chip">✍ <b>${b.percent}%</b>${b.status==='over'?' 초과':''}</span>`;
    }


    function v35CostChipHtml(){
        const c=v35EstimatedAnlas();
        const tone=c.needsAnlasNow?'warn':'';
        const title=`${c.costLine}${c.hint?` · ${c.hint}`:''}`;
        return `<span class="q-chip ${tone}" id="v35-cost-chip" title="${escapeHtml(title)}">💠 <b>${escapeHtml(c.chipText)}</b></span>`;
    }

    function v35CostSuffixHtml(){
        const c=v35EstimatedAnlas();
        if(c.total<=0)return ` <span class="bsub">0 Anlas</span>`;
        if(c.nowUsesBattery)return ` <span class="bsub">Anlas 시 예상 ${c.total}</span>`;
        if(c.batteryUnknown)return ` <span class="bsub">Anlas 기준 예상 ${c.total}</span>`;
        return ` <span class="bsub">예상 ${c.total} Anlas</span>`;
    }

    function v35GenerateToneClass(){
        const c=v35EstimatedAnlas();
        return c.needsAnlasNow?'warnb':'primary';
    }

    function v35GenerateLabelHtml(){
        const c=v35EstimatedAnlas(),b=v35PromptBudget();
        const main=b.status==='over'?'✨ 그래도 생성':(c.needsAnlasNow&&c.family==='v5'?'✨ Anlas로 생성':'✨ 이미지 생성');
        const extra=b.status==='over'?` · 프롬프트 약 ${b.used-b.limit} 토큰 초과`:'';
        return `<span class="bstack">${main}<span class="bsub">${escapeHtml(c.costLine)}${extra}</span></span>`;
    }

    function v35PromptGaugeHtml(){
        const b=v35PromptBudget();
        if(!b.shared)return v35GaugeHtml({
            thin:true,
            percent:Math.min(100,b.percent),
            tone:b.status==='over'?'dgr':(b.status==='tight'?'warn':'ok'),
            foot:`약 ${b.scene} / ${b.limit}`
        });
        const segs=[{percent:b.scene/b.limit*100,tone:'c1'}].concat(
            b.chars.map((c,i)=>({percent:c.tokens/b.limit*100,tone:`c${(i%3)+2}`}))
        );
        if(b.status==='over')segs.push({percent:(b.used-b.limit)/b.limit*100,tone:'over'});
        return v35GaugeHtml({
            segments:segs,
            foot:`약 ${b.used} / ${b.limit} 합산 · 장면 ${b.scene}${b.chars.map(c=>` · ${escapeHtml(c.name)} ${c.tokens}`).join('')}`,
            tone:b.status==='over'?'dgr':(b.status==='tight'?'warn':'ok')
        });
    }

    function v35GenerateInspectorSummaryHtml(){
        const v=v35QuotaView(),b=v35PromptBudget(),c=v35EstimatedAnlas();
        const width=Math.max(1,Number(cspV35Session?.generation?.width||832));
        const height=Math.max(1,Number(cspV35Session?.generation?.height||1216));
        const steps=Math.max(1,Number(cspV35Session?.generation?.steps||28));
        const samples=Math.max(1,Number(cspV35Session?.generation?.nSamples||cspV35Session?.generation?.n_samples||1));
        const costBadge=c.total<=0?'0 Anlas':`${c.total} Anlas`;
        const costBadgeTone=c.total<=0?'o':(c.needsAnlasNow?'w':'o');
        const costFoot=[`${width} × ${height}`,`${steps} steps`,samples>1?`${samples}장`:'1장',c.family==='v5'?'V5 예상가는 V4.5 기준의 1.5배':'V4.5 예상가'].join(' · ');
        const costNote=[c.costLine,c.preciseExtra>0?`Precise Reference ${c.refCount}장 +${c.preciseExtra} Anlas`:'',c.baseGenerationFree?'Opus 무료 조건으로 기본 생성비는 0':''].filter(Boolean).join(' · ');
        const costCard=`<div class="card"><div class="hd">💠 예상 생성 비용<div class="grow"></div><span class="badge ${costBadgeTone}">${escapeHtml(costBadge)}</span></div><div class="gfoot">${escapeHtml(costFoot)}</div><div class="gfoot ${c.needsAnlasNow?'warn':''}">${escapeHtml(costNote)}</div></div>`;
        if(v.mode==='loading')return `${costCard}<div class="card"><div class="hd">🔋 V5 배터리</div><div class="gfoot">확인 중…</div></div><div class="card"><div class="hd">👥 캐릭터 슬롯<div class="grow"></div><span class="badge">${b.charCount} / ${b.maxCharacters}명</span></div>${v35GaugeHtml({thin:true,percent:b.charCount/b.maxCharacters*100,tone:'ok'})}</div>`;
        if(v.mode==='anlas')return `${costCard}<div class="card"><div class="hd">🔋 V5 배터리<div class="grow"></div><span class="badge">${escapeHtml(v.tier||'')}</span></div><div class="gfoot">Opus 등급이 아니라 V5 배터리는 적용되지 않아.</div></div><div class="card"><div class="hd">👥 캐릭터 슬롯<div class="grow"></div><span class="badge">${b.charCount} / ${b.maxCharacters}명</span></div>${v35GaugeHtml({thin:true,percent:b.charCount/b.maxCharacters*100,tone:'ok'})}</div>`;
        if(v.mode==='unknown')return `${costCard}<div class="card"><div class="hd">🔋 V5 배터리<div class="grow"></div><span class="badge">확인 불가</span></div><div class="gfoot">구독 정보를 읽지 못했어. 생성은 그대로 돼.</div><button class="btn tiny" id="v35-quota-refresh">지금 다시 확인</button></div><div class="card"><div class="hd">👥 캐릭터 슬롯<div class="grow"></div><span class="badge">${b.charCount} / ${b.maxCharacters}명</span></div>${v35GaugeHtml({thin:true,percent:b.charCount/b.maxCharacters*100,tone:'ok'})}</div>`;
        ...
        return `${costCard}<div class="card"><div class="hd">🔋 V5 배터리<div class="grow"></div><span class="badge ${tone==='ok'?'o':(tone==='warn'?'w':'d')}">${v.status==='exhausted'?'소진':(v.status==='low'?'부족':'정상')}</span></div>
${v35GaugeHtml({label:`<b>${v.percent}%</b> 남음`,right:v.imagesLeft!==null?`약 ${v.imagesLeft.toLocaleString()}장`:'' ,percent:v.percent,tone,foot})}
<button class="btn tiny" id="v35-quota-refresh">지금 다시 확인</button></div>
<div class="card"><div class="hd">👥 캐릭터 슬롯<div class="grow"></div><span class="badge">${b.charCount} / ${b.maxCharacters}명</span></div>
${v35GaugeHtml({thin:true,percent:b.charCount/b.maxCharacters*100,tone:'ok'})}</div>`;
    }
    ...
    function formatV35CompactNumber(value) {
        ...
        if (n >= 10000) return `${Math.round(n / 100) / 10}만`;
        return Math.round(n).toLocaleString();
    }

    function paintV35NaiUsage(el, account, model) {
        if (!el) return;
        const isV5 = isNaiV5Model(model);
        const label = el.querySelector('.nai-usage-label');
        const pctEl = el.querySelector('.nai-usage-pct');
        const anlasEl = el.querySelector('.nai-usage-anlas');
        const fill = el.querySelector('.nai-usage-fill');
        const quota = account?.quota || {};
        const percent = Number(quota.percent);
        const hasPercent = Number.isFinite(percent);
        const clamped = hasPercent ? Math.max(0, Math.min(100, percent)) : 0;

        el.classList.remove('is-low', 'is-empty');
        if (label) label.textContent = isV5 ? 'V5 할당량' : 'NAI';
        if (fill) fill.style.width = `${clamped}%`;

        if (isV5) {
            if (hasPercent) {
                if (pctEl) pctEl.textContent = `${Math.round(clamped)}%`;
                if (clamped <= 0) el.classList.add('is-empty');
                else if (clamped < 10) el.classList.add('is-low');
            } else if (quota.status === 'not_applicable') {
                if (pctEl) pctEl.textContent = '없음';
            } else {
                if (pctEl) pctEl.textContent = '확인 불가';
            }
        } else if (pctEl) {
            pctEl.textContent = '';
        }

        if (anlasEl) anlasEl.textContent = `Anlas ${formatV35CompactNumber(account?.total || 0)}`;

        const detail = [];
        if (isV5 && hasPercent) {
            detail.push(`V5 무료 할당량 ${Math.round(clamped)}%`);
            if (Number.isFinite(Number(quota.imagesLeft))) detail.push(`예상 약 ${Number(quota.imagesLeft).toLocaleString()}장`);
            if (Number.isFinite(Number(quota.rechargePerHour))) detail.push(`시간당 약 ${quota.rechargePerHour}% 충전`);
        } else if (isV5 && quota.status === 'not_applicable') {
            detail.push('현재 구독에서는 V5 무료 할당량이 적용되지 않음');
        }
        detail.push(`잔여 Anlas ${Number(account?.total || 0).toLocaleString()}`);
        el.title = detail.join(' · ') + ' · 클릭해서 새로고침';
    }

    async function refreshV35NaiUsage(root = cspV35Root, { force = false } = {}) {
        const el = root?.querySelector?.('#v35-nai-usage');
        const session = cspV35Session;
        if (!el || !session) return;
        const global = getGlobalSettings();
        if (!String(global.naiApiKey || '').trim()) {
            const pctEl = el.querySelector('.nai-usage-pct');
            const anlasEl = el.querySelector('.nai-usage-anlas');
            if (pctEl) pctEl.textContent = '미연결';
            if (anlasEl) anlasEl.textContent = 'NAI 토큰 필요';
            el.title = '설정에서 NovelAI Persistent API Token을 입력해줘.';
            return;
        }
        ...
        try {
            ...
            if (cspV35Session !== requestSession || cspV35Root !== requestRoot || !el.isConnected) return;
            paintV35NaiUsage(el, account, v35Model());
        } catch (err) {
            if (cspV35Session !== requestSession || cspV35Root !== requestRoot || !el.isConnected) return;
            const pctEl = el.querySelector('.nai-usage-pct');
            const anlasEl = el.querySelector('.nai-usage-anlas');
            if (pctEl) pctEl.textContent = '오류';
            if (anlasEl) anlasEl.textContent = '계정 조회 실패';
            el.title = `NAI 계정 조회 실패 · ${err?.message || err} · 클릭해서 다시 시도`;
        } finally {
            if (el.isConnected) delete el.dataset.loading;
        }
    }

    async function hydrateV35CurrentImageFromHistory(session = cspV35Session, { force = false } = {}) {
        if (!session || cspV35Session !== session || !session.messageKey || session.__imageHydrating) return;
        if (session.currentImageSrc && !force) return;
        const rec = normalizeSceneRecordHistory(getSceneRecords()[session.messageKey], session.messageKey);
        if (!rec?.history?.length) return;
        const index = session.currentHistoryIndex == null ? clampHistoryIndex(rec) : Math.max(0, Math.min(Number(session.currentHistoryIndex) || 0, rec.history.length - 1));
        session.__imageHydrating = true;
        try {
            const src = await getRecordImageSrc(rec, index);
            if (cspV35Session !== session || !src) return;
            if (force || !session.currentImageSrc) {
                session.currentImageSrc = src;
                session.currentHistoryIndex = index;
                if (cspV35Root?.dataset.view === 'studio') renderV35Studio();
            }
        } catch (err) {
            console.warn('[CSP] studio image hydrate failed', err);
        } finally {
            session.__imageHydrating = false;
        }
    }

    function renderV35Studio(){if(!cspV35Session)return;const s=cspV35Session,{root,app}=mountV35();root.dataset.view='studio';const model=v35Model(),fam=getNaiModelCapability(model).family,uc=normalizeNaiUcPresetForModel(s.generation.ucPreset,model);app.innerHTML=`<div class="v35-top"><div class="v35-brand">🎨 Scene Painter <small>스튜디오</small></div><select class="select" id="v35-q-model" style="width:175px">${buildNaiModelOptionsHtml(model)}</select><select class="select" id="v35-q-size" style="width:145px"><option value="portrait" ${s.generation.width==832&&s.generation.height==1216?'selected':''}>832 × 1216</option><option value="square" ${s.generation.width==1024&&s.generation.height==1024?'selected':''}>1024 × 1024</option><option value="landscape" ${s.generation.width==1216&&s.generation.height==832?'selected':''}>1216 × 832</option><option value="custom">직접 입력</option></select>${fam==='v5'?`<select class="select" id="v35-q-quality" style="width:130px"><option value="none" ${s.generation.v5QualityPreset==='none'?'selected':''}>품질 · 없음</option><option value="light" ${s.generation.v5QualityPreset==='light'?'selected':''}>품질 · 라이트</option><option value="standard" ${s.generation.v5QualityPreset==='standard'?'selected':''}>품질 · 표준</option></select>`:`<button class="btn" id="v35-q-quality-toggle">품질 태그 · ${s.generation.qualityToggle!==false?'켜짐':'꺼짐'}</button>`}<select class="select" id="v35-q-uc" style="width:160px">${buildNaiUcPresetOptionsHtml(uc,model)}</select><div class="grow"></div>${v35QuotaChipHtml()}${v35BudgetChipHtml()}${v35CostChipHtml()}<button class="btn ghost" id="v35-gallery-open">🖼 갤러리</button><button class="btn ghost" id="v35-settings-open">⚙ 설정</button><button class="iconbtn" id="v35-close">×</button></div><div class="studio ${s.tool?'panel':''}"><div class="rail"><button class="iconbtn ${s.tool==='scene'?'active':''}" data-tool="scene" title="장면">🎬</button><button class="iconbtn ${s.tool==='characters'?'active':''}" data-tool="characters" title="캐릭터">👥</button><button class="iconbtn ${s.tool==='context'?'active':''}" data-tool="context" title="장면 정보">🌐</button><div class="grow"></div><button class="iconbtn ${s.tool==='raw'?'active':''}" data-tool="raw" title="원문 / 디버그">⌘</button></div>${s.tool?'<aside class="context-panel" id="v35-tool-panel"></aside>':''}<main class="output"><div class="canvas-wrap"><div class="canvas" id="v35-canvas" style="--iw:${Number(s.generation.width)||832};--ih:${Number(s.generation.height)||1216}">${s.currentImageSrc?`<img src="${escapeHtml(s.currentImageSrc)}" id="v35-main-img"><div class="canvas-tools"><button class="iconbtn glass" id="v35-view">⛶</button><button class="iconbtn glass" id="v35-download">↓</button></div>`:`<div class="canvas-empty"><strong>생성 준비 완료</strong>장면 분석이 끝났어.<br>오른쪽 설정에서 확인하고 생성하면 돼.</div>`}${s.positionMode?v35PositionOverlayHtml(s):''}${!['idle','ready'].includes(s.status)&&!s.error?`<div class="loading"><div class="loading-card"><div class="spinner"></div><strong>${escapeHtml(s.statusMessage||'처리 중…')}</strong></div></div>`:''}</div>${s.error?`<div style="position:absolute;left:24px;right:24px;bottom:24px;padding:12px;border:1px solid var(--danger);border-radius:12px;background:var(--sf);color:var(--danger)">${escapeHtml(s.error)}</div>`:''}</div><div class="film"><div class="film-label">🎞 장면</div><div id="v35-film" style="display:flex;gap:6px;align-items:center"></div><div class="grow"></div><button class="btn ghost tiny" id="v35-film-all">전체 기록 보기 →</button></div></main><aside class="inspector"><div class="inspector-tabs"><button class="tab ${s.inspector==='prompt'?'active':''}" data-inspector="prompt">✍ 프롬프트</button><button class="tab ${s.inspector==='generate'?'active':''}" data-inspector="generate">⚙ 생성</button></div><div class="inspector-body" id="v35-inspector"></div></aside></div><div class="bottom"><button class="btn" id="v35-comic-mode"><svg class="csp-comic-icon" style="display:inline-block;width:16px;height:16px;flex:0 0 16px;vertical-align:-3px;color:inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="3" y="4" width="18" height="16" rx="1.3"/><path d="M7.5 7.5h5a1.5 1.5 0 0 1 1.5 1.5v2a1.5 1.5 0 0 1-1.5 1.5h-2L8 14v-1.5h-.5A1.5 1.5 0 0 1 6 11V9a1.5 1.5 0 0 1 1.5-1.5Z"/><path d="m15 17 3-3m0 3h1"/></svg> 만화</button><button class="btn" id="v35-reanalyze" ${s.markdown?'':'disabled title="원래 메시지가 현재 DOM에 없어서 재분석할 수 없어."'}>↻ 요청 재분석</button><span class="help" id="v35-dirty">${s.promptDirty||s.negativeDirty||s.characterDirty?'● 프롬프트 수정됨':''}</span><div class="nai-usage" id="v35-nai-usage" title="클릭해서 NAI 할당량/Anlas 새로고침"><span class="nai-usage-label">${fam==='v5'?'V5 할당량':'NAI'}</span>${fam==='v5'?'<span class="nai-usage-track"><i class="nai-usage-fill"></i></span>':''}<b class="nai-usage-pct">…</b><span class="nai-usage-anlas">Anlas …</span></div><div class="grow"></div><button class="btn" id="v35-reroll" ${s.currentImageSrc?'':'disabled'}>🎲 같은 설정으로 리롤${v35CostSuffixHtml()}</button><button class="btn ${v35GenerateToneClass()}" id="v35-generate">${v35GenerateLabelHtml()}</button></div>`;bindV35Studio(root);renderV35ToolPanel(root);renderV35Inspector(root);renderV35Film(root);bindV35CanvasFit(root);refreshV35NaiUsage(root);hydrateV35CurrentImageFromHistory(s)}
    function renderV35ToolPanel(root){
        const p=root.querySelector('#v35-tool-panel'),s=cspV35Session;
        if(!p||!s.tool)return;
        const plan=s.plan||{};
        if(s.tool==='scene'){
            const idx=Number(plan.insertAfterParagraph||0);
            p.innerHTML=`<div class="panel-head"><h3>🎬 장면</h3><button class="iconbtn" style="margin-left:auto" data-close-tool>×</button></div>
            <div class="section" style="padding-top:0">
                <div class="section-title">선택 장면</div>
                <div style="padding:12px;border-left:2px solid var(--ac);background:var(--soft);border-radius:0 10px 10px 0">${escapeHtml(plan.visualAnchor||getParagraphTextByIndex(s.markdown,idx)||'선택 장면')}</div>
                <div class="grid2"><div class="field"><label>기준 문단</label><select class="select" id="v35-scene-paragraph">${buildParagraphSelectOptions(s.markdown,idx)}</select></div><div class="field"><label>삽입 위치</label><input class="input" id="v35-scene-insert" type="number" min="0" value="${idx}"></div></div>
            ...
            <div class="section"><div class="section-title">🎥 연출</div><div class="field"><label>톤</label><div>${escapeHtml(plan.director?.tone||plan.mood||'—')}</div></div><details><summary style="cursor:pointer;color:var(--tx2)">상세</summary><div style="display:flex;flex-direction:column;gap:12px;margin-top:12px"><div class="field"><label>시각적 초점</label><div>${escapeHtml(plan.director?.visualFocus||'—')}</div></div><div class="field"><label>공간 연출 의도</label><div>${escapeHtml(plan.director?.spatialIntent||'—')}</div></div><div class="field"><label>조명 연출 의도</label><div>${escapeHtml(plan.director?.lightingIntent||'—')}</div></div></div></details></div>`;
        }else if(s.tool==='characters'){
            const chars=s.promptState.charPrompts||[],model=v35Model(),referenceSupported=supportsNaiPreciseReference(model);
            p.innerHTML=`<div class="panel-head"><h3>👥 등장인물 <span class="badge accent">${chars.length}</span></h3><button class="iconbtn" style="margin-left:auto" data-close-tool>×</button></div>${chars.length?chars.map((c,i)=>{const storedRefs=getStoredReferenceCount([c]);return `<button class="rowitem" data-scene-char="${i}"><span style="width:22px;height:22px;border-radius:50%;display:grid;place-items:center;background:var(--soft);color:var(--ac);font-weight:700">${i+1}</span><span class="main"><span class="name">${escapeHtml(c.name||`캐릭터 ${i+1}`)}</span><span class="meta">${escapeHtml((c.prompt||'').split(',').slice(0,5).join(', '))}</span>${!referenceSupported&&storedRefs?`<span class="meta">레퍼런스 ${storedRefs}개 저장됨 · V5 미지원</span>`:''}</span></button>`}).join(''):'<div class="help">분리된 캐릭터 슬롯 없음</div>'}`;
        }else if(s.tool==='context'){
            const rows=v35ContextRows(plan),idx=Number(plan.insertAfterParagraph||0),snapshot=plan.contextSnapshot||{};
            const targetParagraph=getParagraphTextByIndex(s.markdown,idx)||plan.visualAnchor||'';
            const recentCount=Number(snapshot.recentMessageCount ?? CSP_VISUAL_CONTEXT_LIMITS.recentFactMessages);
            const visualSettingCount=Array.isArray(snapshot.matchedVisualSettingIds)?snapshot.matchedVisualSettingIds.length:0;
            p.innerHTML=`<div class="panel-head"><h3>🌐 장면 정보</h3><button class="iconbtn" style="margin-left:auto" data-close-tool>×</button></div>
            <div class="section" style="padding-top:0"><div class="section-title">📚 참고 범위</div>
                <div class="field"><label>현재 대상 문단</label><div>${escapeHtml(`문단 ${idx+1} · ${String(targetParagraph).replace(/\s+/g,' ').slice(0,120) || '선택 문단'}`)}</div></div>
                <div class="field"><label>현재 턴</label><div>지금 삽화를 만드는 AI 답변 전체를 참고</div></div>
                <div class="field"><label>최근 대화</label><div>현재 턴 이전 최근 ${recentCount}개 메시지를 참고${recentCount>=CSP_VISUAL_CONTEXT_LIMITS.recentFactMessages?' (최대 범위)':''}</div></div>
            </div>
            <div class="section"><div class="section-title">🧭 해석된 장면</div>${rows.length?rows.map(r=>`<div style="display:grid;grid-template-columns:22px 1fr auto;gap:8px;padding:8px 0;border-bottom:1px solid var(--line2)"><div>${r.ic}</div><div>${escapeHtml(r.v)}</div><span class="badge ${['RECENT','TARGET'].includes(r.src)?'accent':''}">${escapeHtml(v35SourceLabel(r.src))}</span></div>`).join(''):'<div class="help">표시할 장면 정보 없음</div>'}</div>
            <div class="section"><div class="section-title">🧩 추가 설정</div><div class="help">세계관 기본 설정 ${s.room.visualContext?.worldProfile?.enabled!==false?'사용':'사용 안 함'}</div><div class="help">적용된 시각 설정 ${visualSettingCount}건</div><div class="help">현재 상태는 최신 장면의 물리 상태만 보조로 참고해.</div></div>`;
        }else{
            p.innerHTML=`<div class="panel-head"><h3>⌘ 원본 정보 / 디버그</h3><button class="iconbtn" style="margin-left:auto" data-close-tool>×</button></div><div class="field"><label>ScenePlan 원본</label><textarea class="ta editor" readonly>${escapeHtml(JSON.stringify(plan,null,2))}</textarea></div>`;
        }
        bindV35ToolPanel(p);v35AugmentCharactersPanel(p);
    }
    function v35ChipsHtml(text, editKey, limit){
        const list=String(text||'').split(',').map(t=>t.trim()).filter(Boolean);
        if(!list.length)return '<div class="prompt-preview">—</div>';
        const max=limit||8;
        const head=list.slice(0,max).map(t=>`<span class="tg">${escapeHtml(t)}</span>`).join('');
        const rest=list.length>max?`<button type="button" class="tg more" data-edit="${editKey}">＋ ${list.length-max}개 더</button>`:'';
        return `<div class="chipview">${head}${rest}</div>`;
    }

    function renderV35Inspector(root){const b=root.querySelector('#v35-inspector'),s=cspV35Session;if(!b)return;const ps=s.promptState||{},chars=ps.charPrompts||[],storedReferenceCount=getStoredReferenceCount(chars);if(s.inspector==='prompt'){b.innerHTML=`<div class="prompt-block"><div class="prompt-head"><b>장면 프롬프트</b><div class="grow"></div><button class="btn ghost tiny" data-edit="scene">편집</button></div>${s.__edit==='scene'?`<textarea class="ta editor" id="v35-prompt-edit">${escapeHtml(ps.finalPrompt||'')}</textarea>`:`${v35ChipsHtml(ps.finalPrompt,'scene',8)}`}${v35PromptGaugeHtml()}</div><div class="prompt-block"><div class="prompt-head"><b>캐릭터</b><span class="badge accent">${chars.length}</span></div>${chars.map((c,i)=>`<button class="rowitem" data-edit-char="${i}"><span class="main"><span class="name">${escapeHtml(c.name||`Character ${i+1}`)}</span>${s.__charEdit===i?'':`<span class="meta">${escapeHtml((c.prompt||'').split(',').slice(0,6).join(', '))}</span>`}</span><span>›</span></button>`).join('')}${Number.isInteger(s.__charEdit)&&chars[s.__charEdit]?`<div class="section"><div class="field"><label>${escapeHtml(chars[s.__charEdit].name||'Character')} Prompt</label><textarea class="ta editor" id="v35-char-prompt">${escapeHtml(chars[s.__charEdit].prompt||'')}</textarea></div>${String(chars[s.__charEdit].uc||'').trim()?`<div class="field"><label>개별 UC</label><textarea class="ta compact" id="v35-char-uc">${escapeHtml(chars[s.__charEdit].uc)}</textarea></div>`:`<details class="slotfold"><summary class="slotline">개별 UC · 없음<span class="grow"></span><span class="badge">＋ 추가</span></summary><textarea class="ta compact" id="v35-char-uc" placeholder="이 캐릭터에만 적용할 네거티브"></textarea></details>`}</div>`:''}${v35PromptGaugeHtml()}</div><div class="prompt-block"><div class="prompt-head"><b>네거티브</b><div class="grow"></div><span class="badge">${escapeHtml(getNaiUcPresetLabel(s.generation.ucPreset,v35Model()).replace(' UC',''))}</span><button class="btn ghost tiny" data-edit="negative">편집</button></div>${s.__edit==='negative'?`<textarea class="ta editor" id="v35-neg-edit">${escapeHtml(ps.finalNegative||'')}</textarea>`:`${ps.finalNegative?v35ChipsHtml(ps.finalNegative,'negative',6):`<div class="prompt-preview">직접 UC 없음 · preset만 적용</div>`}`}${v35PromptGaugeHtml()}</div>${isNaiV5Model(v35Model())?`${(()=>{const cap=s.plan.render?.caption||s.plan.composition?.caption||'';return cap?`<div class="prompt-block"><div class="prompt-head"><b>V5 캡션</b></div><div class="prompt-preview">${escapeHtml(cap)}</div></div>`:`<div class="prompt-block"><div class="slotline is-static">V5 캡션 · 없음</div></div>`})()}`:''}`}else{const m=v35Model(),fam=getNaiModelCapability(m).family;b.innerHTML=`${v35GenerateInspectorSummaryHtml()}<div class="section" style="padding-top:0"><div class="section-title">⚙ 생성 설정</div><div class="field"><label>모델</label><select class="select" id="v35-g-model">${buildNaiModelOptionsHtml(m)}</select>${!supportsNaiPreciseReference(m)&&storedReferenceCount?`<span class="help">레퍼런스 ${storedReferenceCount}개 저장됨 · V5 미지원</span>`:''}</div><div class="grid2"><div class="field"><label>너비</label><input class="input" id="v35-g-width" type="number" value="${Number(s.generation.width)||832}"></div><div class="field"><label>높이</label><input class="input" id="v35-g-height" type="number" value="${Number(s.generation.height)||1216}"></div></div>${fam==='v5'?`<div class="field"><label>품질</label><select class="select" id="v35-g-quality"><option value="none" ${s.generation.v5QualityPreset==='none'?'selected':''}>없음</option><option value="light" ${s.generation.v5QualityPreset==='light'?'selected':''}>라이트</option><option value="standard" ${s.generation.v5QualityPreset==='standard'?'selected':''}>표준</option></select></div>`:`<button class="btn" id="v35-g-quality-toggle">품질 태그 추가 · ${s.generation.qualityToggle!==false?'켜짐':'꺼짐'}</button>`}<div class="field"><label>UC 프리셋</label><select class="select" id="v35-g-uc">${buildNaiUcPresetOptionsHtml(s.generation.ucPreset,m)}</select></div><div class="field"><label>시드</label><input class="input mono" id="v35-g-seed" value="${escapeHtml(s.generation.seed||'')}" placeholder="랜덤"></div></div><div class="section"><button class="btn ghost" id="v35-advanced" style="justify-content:flex-start">고급 설정 <span class="grow"></span><span class="help">${Number(s.generation.steps)||28} 스텝 · ${escapeHtml(s.generation.sampler||'')}</span> ${s.advanced?'⌄':'›'}</button>${s.advanced?`<div class="grid2"><div class="field"><label>스텝</label><input class="input" id="v35-g-steps" type="number" value="${Number(s.generation.steps)||28}"></div><div class="field"><label>가이던스</label><input class="input" id="v35-g-scale" type="number" step="0.1" value="${Number(s.generation.scale)||5}"></div></div><div class="field"><label>샘플러</label><select class="select" id="v35-g-sampler">${v35SamplerOptions(s.generation.sampler)}</select></div><div class="field"><label>노이즈 스케줄</label><select class="select" id="v35-g-noise">${v35NoiseOptions(s.generation.noiseSchedule)}</select></div><div class="field"><label>가이던스 리스케일</label><input class="input" id="v35-g-rescale" type="number" step="0.02" value="${Number(s.generation.guidanceRescale)||0}"></div>`:''}</div>`}v35InjectPromptTools(b);bindV35Inspector(b)}
    ...
    async function renderV35Film(root){
        const box=root.querySelector('#v35-film'),s=cspV35Session;if(!box)return;const rec=s.messageKey?normalizeSceneRecordHistory(getSceneRecords()[s.messageKey],s.messageKey):null,h=rec?.history||[];
        if(!h.length){box.innerHTML='<span class="help">아직 기록 없음</span>';return}
        const current=s.currentHistoryIndex??clampHistoryIndex(rec);box.innerHTML=h.map((_,i)=>`<button class="thumb ${i===current?'active':''}" data-hi="${i}" title="클릭: 보기 · 우클릭: 삭제"><span class="help">…</span></button>`).join('');
    ...
    function showV35RefineRequestDialog({ focusIndex = 0, initialValue = '' } = {}) {
        return new Promise(resolve => {
            ...
            overlay.className = 'refine-backdrop';
            const value = String(initialValue || '').slice(0, 600);
            overlay.innerHTML = `<div class="refine-dialog" role="dialog" aria-modal="true" aria-labelledby="v35-refine-title">
                <div class="refine-head"><b id="v35-refine-title">↻ 요청 재분석</b><div class="grow"></div><button class="iconbtn" id="v35-refine-close">×</button></div>
                <div class="refine-body">
                    <div class="help">현재 장면 분석 결과를 기본안으로 유지하고, 아래 요청과 관련된 부분만 다시 분석해.</div>
                    <div class="refine-context"><span class="badge accent">기준 문단 ${Number(focusIndex) + 1}</span><span class="help">현재 ScenePlan + 현재 턴 + 최근 대화 + 장면 설정을 함께 참고</span></div>
                    <div class="field"><label>추가 요청</label><textarea class="ta default" id="v35-refine-text" maxlength="600" placeholder="예: 상반신 위주로 바꾸고, 정면 시선을 유지해줘. 배경은 현재 장소 그대로.">${escapeHtml(value)}</textarea><div class="help">요청하지 않은 인물·배경·시간·구도는 가능한 그대로 유지해.</div></div>
                </div>
                <div class="refine-actions"><span class="help" id="v35-refine-count">0 / 600</span><div class="grow"></div><button class="btn" id="v35-refine-cancel">취소</button><button class="btn primary connection-test" id="v35-refine-submit">요청 재분석</button></div>
            </div>`;

            const textarea = overlay.querySelector('#v35-refine-text');
            const counter = overlay.querySelector('#v35-refine-count');
            const submit = overlay.querySelector('#v35-refine-submit');

            const finish = result => {
                if (cspV35RefineDialog?.el === overlay) cspV35RefineDialog = null;
                overlay.remove();
                resolve(result);
            };
            cspV35RefineDialog = { el: overlay, close: finish };

            const sync = () => {
                const current = String(textarea?.value || '');
                if (counter) counter.textContent = `${current.length} / 600`;
                if (submit) submit.disabled = !current.trim();
            };

            overlay.querySelector('#v35-refine-close')?.addEventListener('click', () => finish(null));
            overlay.querySelector('#v35-refine-cancel')?.addEventListener('click', () => finish(null));
            submit?.addEventListener('click', () => {
                const request = String(textarea?.value || '').trim();
    ...
    function openV35GalleryMenu(anchor,item){
        ...
        menu.className='v35-menu';
        const r=anchor.getBoundingClientRect();
        menu.style.cssText=`position:fixed;z-index:2147483600;left:${Math.max(8,Math.min(innerWidth-220,r.right-210))}px;top:${Math.max(8,Math.min(innerHeight-260,r.bottom+4))}px;width:210px;padding:4px;border:1px solid var(--line);border-radius:12px;background:var(--sf);box-shadow:var(--shadow)`;
        menu.innerHTML=`<button class="btn ghost" data-a="studio" style="width:100%;justify-content:flex-start">🎨 스튜디오에서 열기</button><button class="btn ghost" data-a="jump" style="width:100%;justify-content:flex-start">↩ 원래 메시지로 이동</button><button class="btn ghost" data-a="download" style="width:100%;justify-content:flex-start">⬇ 다운로드</button><button class="btn danger" data-a="delete" style="width:100%;justify-content:flex-start">삭제</button>`;
        cspV35Root.appendChild(menu);
        ...
        menu.__v35Cleanup=cleanup;

        menu.addEventListener('click',async e=>{
            const actionBtn=e.target.closest('[data-a]');
            if(!actionBtn||!menu.contains(actionBtn))return;
            const a=actionBtn.dataset.a;
            cleanup();

            try{
                if(a==='studio'){
                    await openV35StudioFromRecord(item.messageKey,item.index);
                    return;
                }

                if(a==='download'){
                    const src=await getRecordImageSrc(item.record,item.index);
                    v35다운로드(src,item.title);
                    return;
                }

                if(a==='jump'){
                    closeV35();
                    const key=CSS.escape(item.messageKey);
                    const trigger=document.querySelector(`.csp-message-generate-btn[data-message-key="${key}"],.csp-message-speed-btn[data-message-key="${key}"]`);
                    const el=trigger?(getMessageGroupContainer(trigger)||trigger.closest('[data-message-group-id]')):null;
                    if(!el)throw new Error('원래 메시지를 현재 화면에서 찾지 못했어.');
                    el.scrollIntoView({behavior:'smooth',block:'center'});
                    el.animate?.([{background:'rgba(167,139,250,.18)'},{background:'transparent'}],{duration:1200});
                    return;
                }

                if(a==='delete'){
                    if(!confirm(`"${item.title}" 이미지를 삭제할까요?`))return;
                    await deleteGalleryHistoryImage(item.messageKey,item.index);
                    openV35Gallery();
                }
            }catch(err){
                console.error('[CSP] gallery menu action failed',a,err);
                v35Toast('작업 실패: '+(err?.message||err),'error',0);
            }
        });
    ...
    function openV35Viewer({src,title='장면 이미지',item=null}={}){
        ...
        if(standalone){const x=mountV35('gallery');x.app.innerHTML='<div></div>'}
        cspV35Root.classList.add('viewer-active');
        if(standalone)cspV35Root.classList.add('viewer-standalone');
        cspV35Root.querySelector('.viewer')?.remove();
        const state={scale:1,x:0,y:0,drag:false,sx:0,sy:0,standalone};cspV35Viewer=state;
        const v=document.createElement('div');v.className='viewer';v.innerHTML=`<div class="viewer-panel"><div class="v35-top"><div class="v35-brand">${escapeHtml(title)}</div><span class="badge" id="v35-v-scale">100%</span><div class="grow"></div><button class="btn ghost" id="v35-v-download">다운로드</button>${item?'<button class="btn" id="v35-v-studio">🎨 스튜디오에서 열기</button>':''}<button class="iconbtn" id="v35-v-close">×</button></div><div class="viewer-main"><img class="viewer-img" id="v35-v-img" src="${escapeHtml(src)}"></div><div class="viewer-footer"><span class="help">${escapeHtml(item?.model||'')}</span>${item?.seed?`<span class="help mono" id="v35-v-seed" title="클릭해서 복사">seed ${escapeHtml(String(item.seed))}</span>`:''}<div class="grow"></div><span class="help">휠 확대 · 드래그 이동 · ESC 닫기</span></div></div>`;cspV35Root.appendChild(v);
        const closeViewer=()=>{if(standalone){closeV35()}else{v.remove();cspV35Root?.classList.remove('viewer-active','viewer-standalone');cspV35Viewer=null}};
        const img=v.querySelector('#v35-v-img'),apply=()=>{img.style.transform=`translate(${state.x}px,${state.y}px) scale(${state.scale})`;v.querySelector('#v35-v-scale').textContent=`${Math.round(state.scale*100)}%`};
        v.querySelector('#v35-v-close').onclick=closeViewer;v.querySelector('#v35-v-download').onclick=()=>v35다운로드(src,title);v.querySelector('#v35-v-studio')?.addEventListener('click',()=>openV35StudioFromRecord(item.messageKey,item.index));
        v.addEventListener('mousedown',e=>{if(!e.target.closest('.viewer-panel'))closeViewer()});
        v.querySelector('#v35-v-seed')?.addEventListener('click',ev=>{const el=ev.currentTarget;navigator.clipboard?.writeText(String(item.seed));el.textContent='복사됨';setTimeout(()=>{if(el.isConnected)el.textContent=`seed ${item.seed}`},1400)});
        const main=v.querySelector('.viewer-main');
        img.draggable=false;
        img.addEventListener('dragstart',e=>e.preventDefault());
        main.addEventListener('wheel',e=>{e.preventDefault();state.scale=Math.max(1,Math.min(4,state.scale+(e.deltaY<0?.25:-.25)));if(state.scale===1){state.x=state.y=0}apply()},{passive:false});
        img.addEventListener('dblclick',()=>{state.scale=state.scale===1?2:1;if(state.scale===1){state.x=state.y=0}apply()});
        main.addEventListener('pointerdown',e=>{if(e.button!==0||state.scale<=1||e.target.closest('button'))return;e.preventDefault();state.drag=true;state.sx=e.clientX-state.x;state.sy=e.clientY-state.y;main.setPointerCapture?.(e.pointerId);main.classList.add('is-dragging')});
        main.addEventListener('pointermove',e=>{if(!state.drag)return;e.preventDefault();state.x=e.clientX-state.sx;state.y=e.clientY-state.sy;apply()});
        const endDrag=e=>{if(!state.drag)return;state.drag=false;main.classList.remove('is-dragging');try{if(e?.pointerId!=null&&main.hasPointerCapture?.(e.pointerId))main.releasePointerCapture(e.pointerId)}catch(_){}};
        main.addEventListener('pointerup',endDrag);main.addEventListener('pointercancel',endDrag);main.addEventListener('lostpointercapture',()=>{state.drag=false;main.classList.remove('is-dragging')})
    }
    function openV35Settings(page=''){if(page)patchV35Ui('settings',{page});const cur=getV35UiState().settings.page||'connection',{root,app}=mountV35('settings');root.dataset.view='settings';const pages=[['connection','🔌 연결'],['characters','🎭 캐릭터'],['world','🌍 세계관 & 설정'],['generation','🎨 생성'],['storage','💾 저장'],['advanced','🛠 고급']];app.innerHTML=`<div class="v35-top"><div class="v35-brand">⚙ Scene Painter <small>설정</small></div><div class="grow"></div><span class="help" id="v35-save-state">저장됨</span><button class="iconbtn" id="v35-s-close">×</button></div><div class="settings-body"><nav class="settings-nav">${pages.map(([id,l])=>`<button class="navbtn ${cur===id?'active':''}" data-spage="${id}">${l}</button>`).join('')}<div class="grow"></div></nav><main class="settings-content"><div class="settings-page" id="v35-settings-page"></div></main></div>`;root.querySelector('#v35-s-close').onclick=()=>cspV35Session?renderV35Studio():closeV35();root.querySelectorAll('[data-spage]').forEach(b=>b.addEventListener('click',()=>{patchV35Ui('settings',{page:b.dataset.spage});openV35Settings()}));renderV35SettingsPage(root,cur)}
    ...
    function renderV35SettingsPage(root,page){const h=root.querySelector('#v35-settings-page');if(page==='connection')renderV35Connections(h);else if(page==='characters')renderV35Characters(h);else if(page==='world')renderV35World(h);else if(page==='generation')renderV35GenerationDefaults(h);else if(page==='storage')renderV35Storage(h);else renderV35Advanced(h)}
    ...
    function bindV35RefUi(container,slotNameGetter,onChange){const boxes=Array.from(container.querySelectorAll('[data-vref]'));boxes.forEach((box,i)=>{const asset=box.querySelector('[data-ref-asset]'),preview=box.querySelector('[data-ref-preview]');if(asset.value)readReferenceFileAsDataUrl(asset.value).then(data=>{if(data&&preview.isConnected)preview.innerHTML=`<img src="${escapeHtml(data)}" style="width:100%;height:100%;object-fit:contain">`}).catch(()=>{if(preview.isConnected)preview.textContent='불러오기 실패'});box.querySelector('[data-ref-file]')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;try{let handle=await getStoredDirectoryHandle();if(!handle)handle=await chooseImageDirectory();else if(!await ensureDirectoryPermission(handle,'readwrite'))throw new Error('저장 폴더 권한이 없어.');const old=asset.value,name=slotNameGetter?.()||'character',id=await saveReferenceFileToLibrary(file,`${name}_ref${i+1}`);if(old&&old!==id){try{await deleteReferenceFileFromLibrary(old)}catch(_){}}asset.value=id;box.querySelector('[data-ref-name]').value=file.name||id;box.querySelector('[data-ref-enabled]').value='1';onChange?.();v35Toast(`Reference ${i+1} 저장 완료`,'success')}catch(err){v35Toast('Reference 저장 실패: '+err.message,'error',0)}finally{e.target.value=''}});box.querySelector('[data-ref-remove]')?.addEventListener('click',async()=>{const id=asset.value;if(id){try{await deleteReferenceFileFromLibrary(id)}catch(_){}}asset.value='';box.querySelector('[data-ref-name]').value='';box.querySelector('[data-ref-enabled]').value='0';preview.textContent='No image';onChange?.()})})}
    function renderV35Characters(h){
        const g=getGlobalSettings(),r=getRoomSettings(),chars=r.characters||[],charUseCoords=g.naiSettings?.useCoords===true;
        let idx=Number(h.dataset.idx??0);
        if(idx>=chars.length)idx=Math.max(0,chars.length-1);
        const pc=r.pcCharacter||{},pcMode=String(pc.mode||'pov');
        const PC_LABEL={pov:'POV · 화면에 안 나옴',visible:'화면에 등장',auto:'자동 판단'};
        h.innerHTML=`<h2 class="page-title">🎭 캐릭터</h2><div class="page-desc">저장 캐릭터와 주인공 설정을 관리해. 이번 장면에 누가 나오는지는 스튜디오에서 골라.</div>
    <div class="master">
    <div class="list">
    <div class="charposbox">
    <div class="qhead">📍 캐릭터 위치 지정</div>
    <select class="select" id="c-coords"><option value="0" ${!charUseCoords?'selected':''}>자동 / 끄기</option><option value="1" ${charUseCoords?'selected':''}>캐릭터 좌표 사용</option></select>
    <div class="help">좌표를 쓸지 말지만 정해. 실제 위치는 장면마다 스튜디오에서 정해.</div>
    </div>
    <button class="pcbtn ${idx===-1?'active':''}" data-cidx="-1">
    <span class="pchead">주인공<span class="grow"></span><span class="badge">${pc.enabled?'켜짐':'꺼짐'}</span></span>
    <span class="help">${escapeHtml(PC_LABEL[pcMode]||pcMode)}</span>
    </button>
    <div class="railhead">저장 캐릭터 <b>${chars.length}</b></div>
    ${chars.map((c,i)=>`<button class="listbtn ${i===idx?'active':''}" data-cidx="${i}"><span><b>${escapeHtml(getCharacterSlotName(c)||`캐릭터 ${i+1}`)}</b><span class="help" style="display:block">${escapeHtml((c.appearanceTags||c.tags||'').split(',').slice(0,4).join(', '))}</span></span></button>`).join('')}
    <button class="btn" id="s-add-char" style="width:100%;margin-top:8px">＋ 캐릭터 추가</button>
    <div class="qbox">
    <div class="qhead">퀵 슬롯<span class="grow"></span><span class="badge">전역</span></div>
    <div class="help">지금 저장된 캐릭터 구성 전체를 이름 붙여 보관해. 다른 방에서도 불러올 수 있어.</div>
    <select class="select" id="qs-select">${buildQuickSlotOptions(g.characterQuickSlots||[])}</select>
    <input class="input" id="qs-name" placeholder="새 이름으로 저장">
    <div class="qbtns"><button class="btn tiny" id="qs-load">불러오기</button><button class="btn tiny" id="qs-save">현재 구성 저장</button><span class="grow"></span><button class="btn tiny ghost" id="qs-del">삭제</button></div>
    </div>
    </div>
    <div id="s-char-detail"></div>
    </div>`;
        h.querySelectorAll('[data-cidx]').forEach(b=>b.onclick=()=>{h.dataset.idx=b.dataset.cidx;renderV35Characters(h)});
        h.querySelector('#c-coords').addEventListener('change',e=>{
            const gg=getGlobalSettings();
            gg.naiSettings={...getDefaultNaiSettings(),...(gg.naiSettings||{}),useCoords:e.currentTarget.value==='1'};
            saveGlobalSettings(gg);
            v35Saved();
        });
        h.querySelector('#s-add-char').onclick=()=>{
            const rr=getRoomSettings();
            rr.characters=[...(rr.characters||[]),{slotId:`slot-${Date.now().toString(36)}`,name:'',aliases:[],appearanceTags:'',outfitTags:'',tags:'',uc:'',references:[createDefaultReferenceSlot(),createDefaultReferenceSlot()]}];
            saveRoomSettings(rr);
            h.dataset.idx=String(rr.characters.length-1);
            renderV35Characters(h);
        };
        h.querySelector('#qs-save').onclick=()=>{
            const name=h.querySelector('#qs-name').value.trim();
            if(!name){v35Toast('퀵 슬롯 이름을 입력해.','error');return}
            const gg=getGlobalSettings(),list=[...(gg.characterQuickSlots||[])];
            const slot={name,characters:cloneCharacterSlots(getRoomSettings().characters||[]),updatedAt:Date.now()};
            const at=list.findIndex(x=>String(x.name||'').trim()===name);
            if(at>=0)list[at]=slot;else list.push(slot);
            saveGlobalSettings({...gg,characterQuickSlots:list});
            renderV35Characters(h);
            v35Toast(`퀵 슬롯 저장: ${name}`,'success');
        };
        h.querySelector('#qs-load').onclick=()=>{
            const name=h.querySelector('#qs-select').value;
            const slot=getQuickSlotByName(getGlobalSettings().characterQuickSlots||[],name);
            if(!slot){v35Toast('불러올 퀵 슬롯이 없어.','error');return}
            const rr=getRoomSettings();
            rr.characters=cloneCharacterSlots(slot.characters);
            saveRoomSettings(rr);
            h.dataset.idx='0';
            renderV35Characters(h);
            v35Toast(`퀵 슬롯 불러옴: ${name}`,'success');
        };
        h.querySelector('#qs-del').onclick=()=>{
            const name=h.querySelector('#qs-select').value;
            if(!name||!confirm(`퀵 슬롯 "${name}"을 삭제할까요?`))return;
            const gg=getGlobalSettings();
            gg.characterQuickSlots=(gg.characterQuickSlots||[]).filter(x=>String(x.name||'').trim()!==name);
            saveGlobalSettings(gg);
            renderV35Characters(h);
        };
        if(idx<0)renderV35PcDetail(h.querySelector('#s-char-detail'));
        else renderV35CharDetail(h.querySelector('#s-char-detail'),idx);
    }
    ...
    /* ============================================================
     * Desktop App — completion layer
     * Desktop UI with shared core/storage/API layers.
     * ============================================================ */
...
#csp-v35-root .pos-layer{position:absolute;inset:0;z-index:15;pointer-events:auto;background-image:linear-gradient(to right,rgba(255,255,255,.12) 1px,transparent 1px),linear-gradient(to bottom,rgba(255,255,255,.12) 1px,transparent 1px);background-size:33.333% 33.333%;background-color:rgba(4,5,8,.12);cursor:crosshair}
#csp-v35-root .pos-layer.grid5{background-size:25% 25%}
#csp-v35-root .pos-dot{position:absolute;width:28px;height:28px;margin:-14px 0 0 -14px;border:2px solid rgba(255,255,255,.9);border-radius:999px;background:var(--acs);color:white;display:grid;place-items:center;font-weight:700;box-shadow:var(--shadow);cursor:grab;user-select:none;touch-action:none}
#csp-v35-root .pos-dot:active{cursor:grabbing;transform:scale(1.1)}
#csp-v35-root .pos-hud{position:absolute;left:12px;bottom:12px;z-index:16;display:flex;gap:6px;align-items:center;padding:6px 8px;border-radius:8px;background:rgba(12,14,20,.72);color:#e9ebf1;font-size:12px;backdrop-filter:blur(6px)}
#csp-v35-root .tool-actions{display:flex;gap:6px;padding:12px 0;border-top:1px solid var(--line2);margin-top:12px}
...
#csp-v35-root .gauge{display:flex;flex-direction:column;gap:5px}
#csp-v35-root .gtop{display:flex;align-items:baseline;gap:6px;font-size:11.5px}
#csp-v35-root .gtop .glab{color:var(--tx2)}
#csp-v35-root .gtop .gr{margin-left:auto;color:var(--tx3);font-family:var(--mono);font-size:11px}
#csp-v35-root .bar{height:8px;border-radius:99px;background:var(--sf3);overflow:hidden;display:flex}
#csp-v35-root .bar.thin{height:5px}
#csp-v35-root .bar i{display:block;height:100%;background:var(--acs)}
#csp-v35-root .bar i.ok{background:var(--ok)}
#csp-v35-root .bar i.warn{background:var(--warn)}
#csp-v35-root .bar i.dgr{background:var(--danger)}
#csp-v35-root .bar i.c1{background:var(--acs)}
#csp-v35-root .bar i.c2{background:#5aa9e6}
#csp-v35-root .bar i.c3{background:#e6a15a}
#csp-v35-root .bar i.c4{background:#7fc79a}
#csp-v35-root .bar i.over{background:repeating-linear-gradient(45deg,var(--danger) 0 4px,rgba(0,0,0,.25) 4px 8px)}
#csp-v35-root .gfoot{font-size:10.5px;color:var(--tx3)}
#csp-v35-root .gfoot.warn{color:var(--warn)}
#csp-v35-root .gfoot.dgr{color:var(--danger)}
#csp-v35-root .q-chip{height:27px;padding:0 9px;border:1px solid var(--line);border-radius:var(--r1);background:var(--sf2);color:var(--tx2);display:inline-flex;align-items:center;gap:6px;font-size:11.5px;white-space:nowrap;cursor:pointer}
#csp-v35-root .q-chip:hover{background:var(--sf3);color:var(--tx)}
#csp-v35-root .q-chip b{color:var(--tx);font-weight:600}
#csp-v35-root .q-chip.warn{border-color:var(--warn);color:var(--warn)}
#csp-v35-root .q-chip.warn b{color:var(--warn)}
#csp-v35-root .q-chip.dgr{border-color:var(--danger);color:var(--danger)}
#csp-v35-root .q-chip.dgr b{color:var(--danger)}
#csp-v35-root .minibar{width:34px;height:5px;border-radius:99px;background:var(--sf3);overflow:hidden;flex:0 0 auto}
#csp-v35-root .minibar i{display:block;height:100%;background:var(--ok)}
#csp-v35-root .minibar i.warn{background:var(--warn)}
#csp-v35-root .minibar i.dgr{background:var(--danger)}
#csp-v35-root .bstack{display:flex;flex-direction:column;align-items:flex-start;gap:1px;line-height:1.15}
#csp-v35-root .bsub{font-size:9.5px;opacity:.85;font-weight:400}
#csp-v35-root .btn.warnb{background:var(--warn);border-color:transparent;color:#1b1405;font-weight:600}
/* 태그 칩 입력 */
...
/* 좌측 레일 */
#csp-v35-root .pcbtn{width:100%;text-align:left;border:1px solid var(--line);border-radius:var(--r2);background:var(--sf);padding:var(--s3);display:flex;flex-direction:column;gap:var(--s1);margin-bottom:var(--s4)}
#csp-v35-root .pcbtn:hover{background:var(--hover)}
#csp-v35-root .pcbtn.active{background:var(--soft);border-color:transparent}
#csp-v35-root .pchead{display:flex;align-items:center;gap:var(--s2);font-size:13px;font-weight:600}
#csp-v35-root .railhead{font-size:12px;color:var(--tx3);padding:0 var(--s1) var(--s2)}
#csp-v35-root .railhead b{color:var(--tx2)}
#csp-v35-root .qbox{margin-top:var(--s5);padding-top:var(--s4);border-top:1px solid var(--line2);display:flex;flex-direction:column;gap:var(--s3)}
#csp-v35-root .qhead{display:flex;align-items:center;gap:var(--s2);font-size:13px;font-weight:600}
#csp-v35-root .qbtns{display:flex;gap:var(--s2);align-items:center;flex-wrap:wrap}
#csp-v35-root .charposbox{margin-bottom:var(--s4);padding:var(--s3);border:1px solid var(--line);border-radius:var(--r2);background:var(--sf2);display:flex;flex-direction:column;gap:var(--s2)}
...
#csp-v35-root .tag-result{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start;padding:8px 0;border-bottom:1px solid var(--line2);min-width:0}
#csp-v35-root .tag-result-main{min-width:0;display:flex;flex-direction:column;gap:2px}
#csp-v35-root .tag-result-main code{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--tx2)}
#csp-v35-root .tag-result-desc{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;word-break:keep-all;overflow-wrap:anywhere;line-height:16px;color:var(--tx3)}
#csp-v35-root .nai-usage{height:30px;display:flex;align-items:center;gap:7px;padding:0 8px;border:1px solid var(--line);border-radius:var(--r1);background:var(--sf2);color:var(--tx3);cursor:pointer;min-width:0}
#csp-v35-root .nai-usage:hover{background:var(--sf3);color:var(--tx2)}
#csp-v35-root .nai-usage-label{font-size:10px;white-space:nowrap}
#csp-v35-root .nai-usage-track{width:82px;height:6px;border-radius:999px;background:var(--line);overflow:hidden;flex:0 0 auto}
#csp-v35-root .nai-usage-fill{display:block;height:100%;width:0;border-radius:inherit;background:var(--ac);transition:width .22s ease}
#csp-v35-root .nai-usage.is-low .nai-usage-fill{background:var(--warn)}
#csp-v35-root .nai-usage.is-empty .nai-usage-fill{background:var(--danger)}
#csp-v35-root .nai-usage-pct{min-width:32px;font-size:10px;line-height:14px;text-align:right;color:var(--tx2);font-variant-numeric:tabular-nums}
#csp-v35-root .nai-usage-anlas{font-size:10px;white-space:nowrap;color:var(--tx3)}
...
#csp-v35-root .viewer{display:grid;place-items:center;padding:20px;background:transparent}
#csp-v35-root .viewer-panel{width:max-content;max-width:min(980px,calc(100vw - 72px));background:var(--sf);border:1px solid var(--line);border-radius:var(--r3);box-shadow:0 28px 70px rgba(0,0,0,.48);overflow:hidden;display:flex;flex-direction:column}
#csp-v35-root .viewer .v35-top{border-bottom:1px solid var(--line)}
#csp-v35-root .viewer-main{background:#0b0d11;touch-action:none;cursor:grab}
#csp-v35-root .viewer-main.is-dragging{cursor:grabbing}
#csp-v35-root .viewer-img{width:auto;height:auto;max-width:min(980px,calc(100vw - 72px));max-height:calc(100vh - 170px);display:block;-webkit-user-drag:none;user-select:none}
#csp-v35-root .viewer-footer{height:auto;min-height:46px;padding:9px 12px;gap:10px;flex-wrap:wrap}
#csp-v35-root .viewer-footer .mono{font-family:var(--mono);cursor:pointer;border-bottom:1px dashed var(--line)}

#csp-v35-root.viewer-active{background:transparent!important}
#csp-v35-root.viewer-standalone .v35-app{display:none!important}
#csp-v35-root.viewer-standalone{background:transparent!important}
#csp-v35-root.viewer-active:after{display:none!important}

#csp-v35-root .btn.primary.connection-test{height:30px;padding:0 10px}

#csp-v35-root #v35-generate{height:30px;padding:0 10px}

#csp-v35-root .refine-backdrop{position:absolute;inset:0;z-index:2147483550;display:grid;place-items:center;padding:24px;background:rgba(0,0,0,.34)}
#csp-v35-root .refine-dialog{width:min(560px,calc(100vw - 80px));background:var(--sf);border:1px solid var(--line);border-radius:var(--r3);box-shadow:0 24px 70px rgba(0,0,0,.48);overflow:hidden}
#csp-v35-root .refine-head{height:46px;display:flex;align-items:center;gap:10px;padding:0 14px;border-bottom:1px solid var(--line)}
#csp-v35-root .refine-body{display:flex;flex-direction:column;gap:12px;padding:14px}
#csp-v35-root .refine-context{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:9px 10px;border:1px solid var(--line2);border-radius:var(--r2);background:var(--sf2)}
#csp-v35-root .refine-actions{display:flex;align-items:center;gap:8px;padding:10px 14px;border-top:1px solid var(--line)}
#csp-v35-root .refine-dialog textarea{min-height:116px;resize:vertical}
        `;document.head.appendChild(style)
    }

    function v35PositionOverlayHtml(s){
        const chars=s?.promptState?.charPrompts||[];
        if(!chars.length)return '';
        const grid5=getNaiModelCapability(v35Model()).coordinateMode==='grid5';
        return `<div class="pos-layer ${grid5?'grid5':''}" id="v35-pos-layer">${chars.map((c,i)=>{const center=normalizeNaiCharacterCenter(c.center,v35Model())||makeDefaultCenter(i,chars.length);return `<button class="pos-dot" type="button" data-pos-index="${i}" style="left:${center.x*100}%;top:${center.y*100}%" title="${escapeHtml(c.name||`Character ${i+1}`)} · ${center.x.toFixed(2)} / ${center.y.toFixed(2)}">${i+1}</button>`}).join('')}<div class="pos-hud">위치 편집 · ${grid5?'V4.5 5×5 격자':'V5 자유 좌표'} <button class="btn tiny" id="v35-pos-done" type="button">완료</button></div></div>`
    }

    function bindV35PositionOverlay(root){
        const s=cspV35Session,layer=root.querySelector('#v35-pos-layer');if(!s||!layer)return;
        root.querySelector('#v35-pos-done')?.addEventListener('click',e=>{e.stopPropagation();s.positionMode=false;renderV35Studio()});
        const grid5=getNaiModelCapability(v35Model()).coordinateMode==='grid5';
        const setPoint=(dot,e)=>{
            const rect=layer.getBoundingClientRect();let x=(e.clientX-rect.left)/rect.width,y=(e.clientY-rect.top)/rect.height;
            x=Math.max(0,Math.min(1,x));y=Math.max(0,Math.min(1,y));
            if(grid5){x=Math.round(x*4)/4;y=Math.round(y*4)/4}else if(e.shiftKey){x=Math.round(x/0.05)*0.05;y=Math.round(y/0.05)*0.05}
            const idx=Number(dot.dataset.posIndex),char=s.promptState.charPrompts?.[idx];if(!char)return;
            char.center={x:Number(x.toFixed(3)),y:Number(y.toFixed(3))};s.generation.useCoords=true;s.characterDirty=true;
            dot.style.left=`${x*100}%`;dot.style.top=`${y*100}%`;dot.title=`${char.name||`Character ${idx+1}`} · ${x.toFixed(2)} / ${y.toFixed(2)}`;
        };
        layer.querySelectorAll('.pos-dot').forEach(dot=>{
            let dragging=false;
            dot.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();dragging=true;dot.setPointerCapture?.(e.pointerId);setPoint(dot,e)});
            dot.addEventListener('pointermove',e=>{if(dragging)setPoint(dot,e)});
            dot.addEventListener('pointerup',e=>{if(dragging)setPoint(dot,e);dragging=false});dot.addEventListener('pointercancel',()=>dragging=false);
            dot.addEventListener('keydown',e=>{if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();const idx=Number(dot.dataset.posIndex),char=s.promptState.charPrompts?.[idx];if(!char)return;const c=normalizeNaiCharacterCenter(char.center,v35Model())||makeDefaultCenter(idx,s.promptState.charPrompts.length),step=grid5?.25:(e.shiftKey?.05:.01);if(e.key==='ArrowLeft')c.x-=step;if(e.key==='ArrowRight')c.x+=step;if(e.key==='ArrowUp')c.y-=step;if(e.key==='ArrowDown')c.y+=step;char.center=normalizeNaiCharacterCenter(c,v35Model())||c;s.generation.useCoords=true;s.characterDirty=true;renderV35Studio()});
        })
    }

    function v35InjectPromptTools(body){
        if(!body||cspV35Session?.inspector!=='prompt')return;
        const block=document.createElement('div');
        block.className='prompt-block';
        block.innerHTML=`<div class="prompt-head"><b>프롬프트 도구</b><span class="grow"></span><span class="help">Danbooru</span></div><div class="field"><label>Danbooru 태그북 검색</label><div style="display:flex;gap:6px"><input class="input" id="v35-tag-search" placeholder="open mouth / 입 벌림" style="min-width:0;flex:1"><button class="btn" id="v35-tag-search-btn" type="button">검색</button></div><div id="v35-tag-results" class="help">검색 결과는 복사해서 원하는 프롬프트에 붙여넣어.</div></div>`;
        body.appendChild(block);
        const run=async()=>{const q=block.querySelector('#v35-tag-search').value.trim(),out=block.querySelector('#v35-tag-results');if(!q){out.textContent='검색어를 입력해.';return}out.textContent='태그북 검색 중…';try{const result=await searchDanbooruTagbook(q,12),rows=result.rows||[];out.innerHTML=rows.length?rows.map((r,i)=>`<div class="tag-result"><div class="tag-result-main"><code>${escapeHtml(r.tag||r.name||'')}</code><span class="tag-result-desc">${escapeHtml(r.ko||r.translation||r.wiki||'설명 없음')}</span></div><button class="btn tiny" data-copy-tag="${i}">복사</button></div>`).join(''):'결과 없음';out.querySelectorAll('[data-copy-tag]').forEach(btn=>btn.onclick=async()=>{const row=rows[Number(btn.dataset.copyTag)],tag=String(row?.tag||row?.name||'');if(tag){await copyTextToClipboard(tag);v35Toast(`태그 복사: ${tag}`,'success')}})}catch(err){out.textContent='검색 실패 · '+(err?.message||err)}};
        block.querySelector('#v35-tag-search-btn').onclick=run;
        block.querySelector('#v35-tag-search').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();run()}};
    }
    ...
    function renderV35Connections(h){
        ...
        let timer=0;
        const save=()=>{clearTimeout(timer);timer=setTimeout(()=>{saveGlobalSettings(v35ConnectionDraft(h));v35Saved()},250)};
    ...
    function renderV35Storage(h){
        const st=getRoomGalleryStats(),rep=getCspStorageReport(),g=getGlobalSettings();h.innerHTML=`<h2 class="page-title">💾 저장</h2><div class="page-desc">갤러리 기록, 이미지 폴더, 브라우저 저장소를 관리해.</div>
<div class="section" style="padding-top:0">
...
<div class="section">
<div class="section-title">📁 이미지 폴더</div>
<div class="setting-row"><div class="setting-label"><b>자동 폴더 저장</b><span class="help">생성 직후 선택 폴더에도 PNG 저장</span></div><select class="select" id="st-folder-on"><option value="1" ${g.folderSaveEnabled?'selected':''}>켜기</option><option value="0" ${!g.folderSaveEnabled?'selected':''}>끄기</option></select></div>
<div class="folder-foot"><span class="status-line" id="st-folder-status">폴더 상태 확인 중…</span><span class="grow"></span><button class="btn" id="st-folder-pick">폴더 선택</button><button class="btn ghost" id="st-folder-clear">연결 해제</button></div>
</div>
<div class="section">
...
<div class="mlist"><div class="mrow"><div><b>현재 방 이미지 기록 삭제</b><span class="help">이 방에서 만든 ${st.imageCount}장이 사라져. 복구할 수 없어.</span></div><button class="btn tiny danger" id="st-clear">삭제</button></div></div>
        ...
        h.querySelector('#st-gallery').onclick=openV35Gallery;h.querySelector('#st-folder-on').onchange=e=>{v35SaveGlobal({folderSaveEnabled:e.target.value==='1'})};
        const refreshFolder=async()=>{const el=h.querySelector('#st-folder-status');try{const handle=await getStoredDirectoryHandle();el.textContent=handle?`● 연결됨 · ${handle.name||'선택한 폴더'}`:'○ 선택된 폴더 없음'}catch(err){el.textContent='! 폴더 상태 확인 실패 · '+(err?.message||err)}};refreshFolder();
        h.querySelector('#st-folder-pick').onclick=async()=>{try{const handle=await chooseImageDirectory();v35SaveGlobal({folderSaveEnabled:true});v35Toast(`폴더 연결: ${handle.name||'완료'}`,'success');renderV35Storage(h)}catch(err){v35Toast('폴더 선택 실패: '+(err?.message||err),'error',0)}};
        h.querySelector('#st-folder-clear').onclick=async()=>{if(!confirm('이미지 저장 폴더 연결을 해제할까요?'))return;await deleteStoredDirectoryHandle();v35SaveGlobal({folderSaveEnabled:false});renderV35Storage(h)};
    ...
    function renderV35Advanced(h){
        ...
        const run=async()=>{const q=h.querySelector('#a-tag-q').value.trim(),out=h.querySelector('#a-tag-result');if(!q){out.textContent='검색어를 입력해.';return}out.textContent='검색 중…';try{const r=await searchDanbooruTagbook(q,20),rows=r.rows||[];out.innerHTML=rows.length?rows.map((x,i)=>`<div class="tag-result"><code>${escapeHtml(x.tag||x.name||'')}</code><span>${escapeHtml(x.ko||x.translation||x.wiki||'')}</span><button class="btn tiny" data-atag="${i}">복사</button></div>`).join(''):'결과 없음';out.querySelectorAll('[data-atag]').forEach(b=>b.onclick=async()=>{const x=rows[Number(b.dataset.atag)],tag=String(x?.tag||x?.name||'');await copyTextToClipboard(tag);v35Toast(`태그 복사: ${tag}`,'success')})}catch(err){out.textContent='검색 실패 · '+(err?.message||err)}};h.querySelector('#a-tag-run').onclick=run;h.querySelector('#a-tag-q').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();run()}};
    ...
    function v35AugmentCharactersPanel(p){
        const s=cspV35Session;if(!p||s?.tool!=='characters')return;const chars=s.promptState?.charPrompts||[];if(!chars.length)return;
        const actions=document.createElement('div');actions.className='tool-actions';actions.innerHTML=`<button class="btn ${s.positionMode?'primary':''}" id="v35-position-edit">${s.positionMode?'위치 편집 완료':'이미지 위에서 위치 편집'}</button><span class="help">${getNaiModelCapability(v35Model()).coordinateMode==='grid5'?'V4.5 · 5×5':'V5 · 자유 좌표'}</span>`;p.appendChild(actions);
        actions.querySelector('#v35-position-edit').onclick=()=>{s.positionMode=!s.positionMode;if(s.positionMode)s.generation.useCoords=true;renderV35Studio()};
        p.querySelectorAll('[data-scene-char]').forEach(b=>b.onclick=()=>{s.__charEdit=Number(b.dataset.sceneChar);s.inspector='prompt';renderV35Studio()});
    }

    function installV35DesktopUi() {
        ensureV35Styles();
    ...
    const CSP_COMIC_MOBILE_UI = false;
...
<div class="v35m-action">${s.busy?'<button type="button" class="btn danger wide" id="cc-cancel">■ 작업 취소</button>':`<button type="button" class="btn" id="cc-defaults" aria-label="현재 옵션을 기본값으로" title="옵션을 기본값으로">☆</button><button type="button" class="btn ${next==='analyze'?'primary':''}" id="cc-analyze" ${s.restoredOnly?'disabled':''}>${p?'↻ 콘티':'✦ 콘티 만들기'}</button><button type="button" class="btn ${next==='generate'?'primary':''}" id="cc-generate" ${canGenerate(s)?'':'disabled'}>${s.imageSrc?'<svg class="csp-comic-icon" style="display:inline-block;width:16px;height:16px;flex:0 0 16px;vertical-align:-3px;color:inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="3" y="4" width="18" height="16" rx="1.3"/><path d="M7.5 7.5h5a1.5 1.5 0 0 1 1.5 1.5v2a1.5 1.5 0 0 1-1.5 1.5h-2L8 14v-1.5h-.5A1.5 1.5 0 0 1 6 11V9a1.5 1.5 0 0 1 1.5-1.5Z"/><path d="m15 17 3-3m0 3h1"/></svg> 다시 생성':'<svg class="csp-comic-icon" style="display:inline-block;width:16px;height:16px;flex:0 0 16px;vertical-align:-3px;color:inherit" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="3" y="4" width="18" height="16" rx="1.3"/><path d="M7.5 7.5h5a1.5 1.5 0 0 1 1.5 1.5v2a1.5 1.5 0 0 1-1.5 1.5h-2L8 14v-1.5h-.5A1.5 1.5 0 0 1 6 11V9a1.5 1.5 0 0 1 1.5-1.5Z"/><path d="m15 17 3-3m0 3h1"/></svg> 만화 1장 생성'}</button>`}</div>`;
    ...
    function start() {
        ...
        migrateLegacySecretsToUserscriptStorage();
